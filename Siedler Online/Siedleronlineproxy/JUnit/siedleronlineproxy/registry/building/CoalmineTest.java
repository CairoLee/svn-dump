...
public class CoalmineTest {
    ...
    public void testBuildingProperties() {
        ...
        assertEquals("Kohlemine", this._object.getName());
        assertEquals(Building.BuildingTypes.COALMINE, this._object.getType());
        assertEquals(Building.BuildingCategory.MINE, this._object.category);
        assertEquals(1 * 60 + 30,this._object.getCycleTime());
    }
    
    @Test
    public void testNeeds() {
        assertSame(1, this._object.getNeeds().size());
        assertTrue(this._object.getNeeds().containsKey(Products.COALDEPOSIT));
        assertSame(1 , this._object.getNeeds().get(Products.COALDEPOSIT));
    }
