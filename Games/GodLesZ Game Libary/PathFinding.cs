...
namespace GLibary {
	class PathFinding {
		...
		**********************************************************/
		public static bool IntersectionOfTwoLines( Vector3 a, Vector3 b, Vector3 c,
					...
					if( CheckObstacles( Location, EndPoint ) == false ) {
						...
						while( true ) {
							while( GetClosestVisiblePointToGoal( NextPoint ) ) {
								...
								// get prev point
								Vector3 temp = closest.Points[ ClosestObjectIndex == 0 ? closest.Points.Count - 1 : ClosestVertexIndex - 1 ];
								//get next point
								Vector3 temp2 = closest.Points[ ClosestObjectIndex == closest.Points.Count - 1 ? 0 : ClosestVertexIndex + 1 ];
							...
							if( Path.Count > 1 ) {
								...
								if( ObstacleList.Count > 0 )
									if( ObstacleList[ 0 ].Points.Count > 2 ) {
										Obstacle clostestObstacle = ObstacleList[ ClosestObjectIndex ];
										// get prev point
										Vector3 temp = clostestObstacle.Points[ ClosestVertexIndex == 0 ? clostestObstacle.Points.Count - 1 : ClosestVertexIndex - 1 ];
										//get next point
										Vector3 temp2 = clostestObstacle.Points[ ClosestVertexIndex == clostestObstacle.Points.Count - 1 ? 0 : ClosestVertexIndex + 1 ];
						...
						if( ObstacleList.Count > 0 )
							if( ObstacleList[ 0 ].Points.Count > 2 ) {
								...
								// get prev point
								Vector3 temp = obstacle.Points[ ClosestVertexIndex == 0 ? obstacle.Points.Count - 1 : ClosestVertexIndex - 1 ];
								//get next point
								Vector3 temp2 = obstacle.Points[ ClosestVertexIndex == obstacle.Points.Count - 1 ? 0 : ClosestVertexIndex + 1 ];
				...
				if( RunPath ) {
					if( FoundPath == true ) {
						...
						if( Location != Path[ NextPointIndex ] ) {
							...
							if( Vector3.Distance( Location, Path[ NextPointIndex ] ) <= Speed ) {
