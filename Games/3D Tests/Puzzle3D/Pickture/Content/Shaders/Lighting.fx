...
Ash_VS_Output Ash_VS( float4 vPos : POSITION, float3 vNormal : NORMAL, float2 vTexCoord0 : TEXCOORD0, float3 vTangent : TANGENT ) {
...
float4 Ash_Diffuse( float3 vectorToLight, float3 vectorToViewer, float3 normal ) {
    return saturate( dot( vectorToLight, normal ) * dot( vectorToViewer, normal ) * DiffuseReflectance * ( 1 - SpecularReflectance ) );
}

float4 Ash_Specular( float3 vectorToLight, float3 vectorToViewer, float3 normal, float3 tangent, float3 binormal, float4 specularColor ) {
    ...
    float numeratorPower = (PhongExponentU * hDotT * hDotT) + (PhongExponentV * hDotB * hDotB);
    numeratorPower /= (1 - (hDotN * hDotN));
    
    float numerator = pow(hDotN, numeratorPower);
    float denominator = dot(halfVector, vectorToLight) * max(dot(normal, vectorToLight), dot(normal, vectorToViewer));
    float fresnelTerm = SpecularReflectance + ((1 - SpecularReflectance) * pow((1 - dot(vectorToLight, halfVector)), 5));

    return (ashSpecularConstantTerm * numerator * specularColor * fresnelTerm) / denominator;
}
...
float4 Ash_General_PS( Ash_VS_Output In, float4 diffuseColor ) {
    ...
    color += Ash_Specular(In.vertexToLight, In.vertexToViewer, normal, tangent, binormal, diffuseColor);
