...
namespace GrfEditor.Library.Controls {
	...
	/// </list>
	/// </remarks>
	//[Designer("GrfEditor.Library.Controls.Design.ObjectListViewDesigner")] // Uncomment this if you are feeling brave :)
	public partial class ObjectListView : ListView, ISupportInitialize {
		...
		/// </summary>
		[Editor("GrfEditor.Library.Controls.Design.OLVColumnCollectionEditor", "System.Drawing.Design.UITypeEditor")]
		//[Editor(typeof(GrfEditor.Library.Controls.Design.OLVColumnCollectionEditor), typeof(System.Drawing.Design.UITypeEditor))]
		...
		public virtual void RemoveIndizies(List<int> modelObjects) {
			if (this.InvokeRequired) {
				this.Invoke((MethodInvoker)delegate() { this.RemoveIndizies(modelObjects); });
				return;
			}
			if (modelObjects == null)
				return;
			...
			try {
				// Give the world a chance to cancel or change the added objects
				ItemsRemovingIndiziesEventArgs args = new ItemsRemovingIndiziesEventArgs(modelObjects);
				...
				this.TakeOwnershipOfObjects();
				ArrayList ourObjects = (ArrayList)this.Objects;
				foreach (int index in modelObjects) {
					if (index >= 0) {
						ourObjects.RemoveAt(index);
