...
namespace GodLesZ.Library.Logging.Config {
	/// <summary>
	/// Use this class to initialize the GodLesZ.Library.Logging environment using an Xml tree.
	/// </summary>
	/// <remarks>
	/// <para>
	...
	public sealed class XmlConfigurator {
...
#if !NETCF
		/// <summary>
		/// Automatically configures the GodLesZ.Library.Logging system based on the 
		...
		/// <c>GodLesZ.Library.Logging</c> that contains the configuration data.
		/// </para>
		/// <para>
		/// To use this method to configure GodLesZ.Library.Logging you must specify 
		/// the <see cref="Log4NetConfigurationSectionHandler"/> section
		/// handler for the <c>GodLesZ.Library.Logging</c> configuration section. See the
		...
		static private void InternalConfigure(ILoggerRepository repository) {
			...
			try {
...
#if NET_2_0
				configElement = System.Configuration.ConfigurationManager.GetSection("GodLesZ.Library.Logging") as XmlElement;
#else
				configElement = System.Configuration.ConfigurationSettings.GetConfig("GodLesZ.Library.Logging") as XmlElement;
#endif
				if (configElement == null) {
					// Failed to load the xml config using configuration settings handler
					LogLog.Error(declaringType, "Failed to find configuration section 'GodLesZ.Library.Logging' in the application's .config file. Check your .config file for the <GodLesZ.Library.Logging> and <configSections> elements. The configuration section should look like: <section name=\"GodLesZ.Library.Logging\" type=\"GodLesZ.Library.Logging.Config.Log4NetConfigurationSectionHandler,GodLesZ.Library.Logging\" />");
				} else {
				...
				if (confEx.BareMessage.IndexOf("Unrecognized element") >= 0) {
					...
					string configSectionStr = "<section name=\"GodLesZ.Library.Logging\" type=\"GodLesZ.Library.Logging.Config.Log4NetConfigurationSectionHandler," + Assembly.GetExecutingAssembly().FullName + "\" />";
		...
		/// <summary>
		/// Configures GodLesZ.Library.Logging using a <c>GodLesZ.Library.Logging</c> element
		/// </summary>
		/// <remarks>
		/// <para>
		/// Loads the GodLesZ.Library.Logging configuration from the XML element
...
#if !NETCF
		/// <summary>
		/// Configures GodLesZ.Library.Logging using the specified configuration file.
		/// </summary>
		/// <param name="configFile">The XML file to load the configuration from.</param>
		/// <remarks>
		/// <para>
		/// The configuration file must be valid XML. It must contain
		/// at least one element called <c>GodLesZ.Library.Logging</c> that holds
		/// the GodLesZ.Library.Logging configuration data.
		/// </para>
		/// <para>
		/// The GodLesZ.Library.Logging configuration file can possible be specified in the application's
		...
		/// a configuration section for the <c>GodLesZ.Library.Logging</c> element otherwise .NET will 
		/// complain. Set the type for the section handler to <see cref="System.Configuration.IgnoreSectionHandler"/>, for example:
		/// <code lang="XML" escaped="true">
		/// <configSections>
		///		<section name="GodLesZ.Library.Logging" type="System.Configuration.IgnoreSectionHandler" />
		///	</configSections>
		/// </code>
		/// </para>
		/// <example>
		/// The following example configures GodLesZ.Library.Logging using a configuration file, of which the 
		...
		/// ...
		/// 
		/// XmlConfigurator.Configure(new FileInfo(ConfigurationSettings.AppSettings["GodLesZ.Library.Logging-config-file"]));
		/// </code>
		/// <para>
		/// In the <c>.config</c> file, the path to the GodLesZ.Library.Logging can be specified like this :
		/// </para>
		/// <code lang="XML" escaped="true">
		/// <configuration>
		///		<appSettings>
		///			<add key="GodLesZ.Library.Logging-config-file" value="log.config"/>
		///		</appSettings>
		///	</configuration>
		/// </code>
		/// </remarks>
		...
		/// <summary>
		/// Configures GodLesZ.Library.Logging using the specified configuration URI.
		/// </summary>
		...
		/// <summary>
		/// Configures GodLesZ.Library.Logging using the specified configuration data stream.
		/// </summary>
		...
		static private void InternalConfigure(ILoggerRepository repository, Stream configStream) {
			...
			if (configStream == null) {
				...
				if (doc != null) {
					...
					// Configure using the 'GodLesZ.Library.Logging' element
					XmlNodeList configNodeList = doc.GetElementsByTagName("GodLesZ.Library.Logging");
					if (configNodeList.Count == 0) {
						LogLog.Debug(declaringType, "XML configuration does not contain a <GodLesZ.Library.Logging> element. Configuration Aborted.");
					} else if (configNodeList.Count > 1) {
						LogLog.Error(declaringType, "XML configuration contains [" + configNodeList.Count + "] <GodLesZ.Library.Logging> elements. Only one is allowed. Configuration Aborted.");
					} else {
		...
		/// <summary>
		/// Configures GodLesZ.Library.Logging using the file specified, monitors the file for changes 
		...
		/// </para>
		/// <para>
		/// For more information on how to configure GodLesZ.Library.Logging using
		...
		/// <summary>
		/// Configures the specified repository using a <c>GodLesZ.Library.Logging</c> element.
		/// </summary>
