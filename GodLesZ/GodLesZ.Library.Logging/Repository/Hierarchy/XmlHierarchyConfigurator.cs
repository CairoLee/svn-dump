...
using GodLesZ.Library.Logging.ObjectRenderer;

namespace GodLesZ.Library.Logging.Repository.Hierarchy {
	/// <summary>
	/// Initializes the GodLesZ.Library.Logging environment using an XML DOM.
	/// </summary>
	/// <remarks>
	/// <para>
	...
	public class XmlHierarchyConfigurator {
					...
					if (currentNode.NodeType == XmlNodeType.Element) {
						...
						if (currentElement.LocalName == APPENDER_REF_TAG) {
							...
							if (appenderContainer != null) {
								...
								LogLog.Error(declaringType, "Requesting attachment of appender named [" + refName + "] to appender named [" + appender.Name + "] which does not implement GodLesZ.Library.Logging.Core.IAppenderAttachable.");
							}
						} else {
		...
		private const string CONFIGURATION_TAG = "GodLesZ.Library.Logging";
