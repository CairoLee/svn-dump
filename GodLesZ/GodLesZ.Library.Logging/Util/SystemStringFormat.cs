	...
	public sealed class SystemStringFormat {
		...
		private static string StringFormat(IFormatProvider provider, string format, params object[] args) {
			try {
				...
				GodLesZ.Library.Logging.Util.LogLog.Warn(declaringType, "Exception while rendering format [" + format + "]", ex);
				return StringFormatError(ex, format, args);
			}
#if !NET_2_0 && !MONO_2_0
			catch
			{
				GodLesZ.Library.Logging.Util.LogLog.Warn(declaringType, "Exception while rendering format ["+format+"]");
		...
		private static string StringFormatError(Exception formatException, string format, object[] args) {
			try {
				StringBuilder buf = new StringBuilder("<GodLesZ.Library.Logging.Error>");
				...
				buf.Append("</args>");
				buf.Append("</GodLesZ.Library.Logging.Error>");

				return buf.ToString();
			} catch (Exception ex) {
				GodLesZ.Library.Logging.Util.LogLog.Error(declaringType, "INTERNAL ERROR during StringFormat error handling", ex);
				return "<GodLesZ.Library.Logging.Error>Exception during StringFormat. See Internal Log.</GodLesZ.Library.Logging.Error>";
			}
#if !NET_2_0 && !MONO_2_0
			catch
			{
				GodLesZ.Library.Logging.Util.LogLog.Error(declaringType, "INTERNAL ERROR during StringFormat error handling");
				return "<GodLesZ.Library.Logging.Error>Exception during StringFormat. See Internal Log.</GodLesZ.Library.Logging.Error>";
			}
#endif
		}
