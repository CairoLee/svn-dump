...
namespace GodLesZ.Library.Logging.Core {
	...
	/// This mapping is held separately for each <see cref="GodLesZ.Library.Logging.Repository.ILoggerRepository"/>.
	...
	public sealed class LevelMap {
		...
		public void Add(string name, int value, string displayName) {
			...
			if (name.Length == 0) {
				throw GodLesZ.Library.Logging.Util.SystemInfo.CreateArgumentOutOfRangeException("name", name, "Parameter: name, Value: [" + name + "] out of range. Level name must not be empty");
			}
