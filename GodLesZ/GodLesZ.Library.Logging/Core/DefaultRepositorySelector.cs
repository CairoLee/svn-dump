...
using GodLesZ.Library.Logging.Config;
...
namespace GodLesZ.Library.Logging.Core {
	...
	public class DefaultRepositorySelector : IRepositorySelector {
		...
		/// an appropriate value would be <see cref="GodLesZ.Library.Logging.Repository.Hierarchy.Hierarchy"/>.
		/// </para>
		/// </remarks>
		...
		public DefaultRepositorySelector(Type defaultRepositoryType) {
			...
			if (!(typeof(ILoggerRepository).IsAssignableFrom(defaultRepositoryType))) {
				throw GodLesZ.Library.Logging.Util.SystemInfo.CreateArgumentOutOfRangeException("defaultRepositoryType", defaultRepositoryType, "Parameter: defaultRepositoryType, Value: [" + defaultRepositoryType + "] out of range. Argument must implement the ILoggerRepository interface");
			}
		...
		/// to create can be overridden by specifying the <see cref="GodLesZ.Library.Logging.Config.RepositoryAttribute"/> 
		/// attribute on the <paramref name="repositoryAssembly"/>.
		/// </para>
		/// <para>
		/// The default values are to use the <see cref="GodLesZ.Library.Logging.Repository.Hierarchy.Hierarchy"/> 
		...
		/// any <see cref="GodLesZ.Library.Logging.Config.ConfiguratorAttribute"/> attributes defined on
		...
		/// <see cref="GodLesZ.Library.Logging.Config.RepositoryAttribute"/> attribute on the 
		...
		/// configured using any <see cref="GodLesZ.Library.Logging.Config.ConfiguratorAttribute"/> 
		...
		/// Also the <see cref="GodLesZ.Library.Logging.Config.RepositoryAttribute"/> attribute on the
		...
		/// </summary>
		/// <param name="assembly">The assembly that has a <see cref="GodLesZ.Library.Logging.Config.RepositoryAttribute"/>.</param>
		...
		private void GetInfoForAssembly(Assembly assembly, ref string repositoryName, ref Type repositoryType) {
			...
			try {
				...
				object[] repositoryAttributes = Attribute.GetCustomAttributes(assembly, typeof(GodLesZ.Library.Logging.Config.RepositoryAttribute), false);
				if (repositoryAttributes == null || repositoryAttributes.Length == 0) {
					...
					if (repositoryAttributes.Length > 1) {
						LogLog.Error(declaringType, "Assembly [" + assembly + "] has multiple GodLesZ.Library.Logging.Config.RepositoryAttribute assembly attributes. Only using first occurrence.");
					}

					GodLesZ.Library.Logging.Config.RepositoryAttribute domAttr = repositoryAttributes[0] as GodLesZ.Library.Logging.Config.RepositoryAttribute;
		...
		/// </summary>
		/// <param name="assembly">The assembly containing <see cref="GodLesZ.Library.Logging.Config.ConfiguratorAttribute"/>
		...
		private void ConfigureRepository(Assembly assembly, ILoggerRepository repository) {
			...
			object[] configAttributes = Attribute.GetCustomAttributes(assembly, typeof(GodLesZ.Library.Logging.Config.ConfiguratorAttribute), false);
			if (configAttributes != null && configAttributes.Length > 0) {
				...
				foreach (GodLesZ.Library.Logging.Config.ConfiguratorAttribute configAttr in configAttributes) {
					if (configAttr != null) {
						try {
			...
			if (repository.Name == DefaultRepositoryName) {
				...
				string repositoryConfigFile = SystemInfo.GetAppSetting("GodLesZ.Library.Logging.Config");
				if (repositoryConfigFile != null && repositoryConfigFile.Length > 0) {
					string applicationBaseDirectory = null;
					try {
						applicationBaseDirectory = SystemInfo.ApplicationBaseDirectory;
					} catch (Exception ex) {
						LogLog.Warn(declaringType, "Exception getting ApplicationBaseDirectory. appSettings GodLesZ.Library.Logging.Config path [" + repositoryConfigFile + "] will be treated as an absolute URI", ex);
					}
...
#if NET_2_0 || MONO_2_0
					Boolean.TryParse(SystemInfo.GetAppSetting("GodLesZ.Library.Logging.Config.Watch"), out watchRepositoryConfigFile);
#else
									{
										string watch = SystemInfo.GetAppSetting("GodLesZ.Library.Logging.Config.Watch");
					...
					if (watchRepositoryConfigFile) {
						...
						try {
							...
							LogLog.Error(declaringType, "DefaultRepositorySelector: Exception while parsing GodLesZ.Library.Logging.Config file physical path [" + repositoryConfigFilePath + "]", ex);
						}
						try {
						...
						try {
							...
							LogLog.Error(declaringType, "Exception while parsing GodLesZ.Library.Logging.Config file path [" + repositoryConfigFile + "]", ex);
						}
		...
		private void LoadPlugins(Assembly assembly, ILoggerRepository repository) {
			...
			object[] configAttributes = Attribute.GetCustomAttributes(assembly, typeof(GodLesZ.Library.Logging.Config.PluginAttribute), false);
			if (configAttributes != null && configAttributes.Length > 0) {
				foreach (GodLesZ.Library.Logging.Plugin.IPluginFactory configAttr in configAttributes) {
					try {
		...
		private void LoadAliases(Assembly assembly, ILoggerRepository repository) {
			...
			object[] configAttributes = Attribute.GetCustomAttributes(assembly, typeof(GodLesZ.Library.Logging.Config.AliasRepositoryAttribute), false);
			if (configAttributes != null && configAttributes.Length > 0) {
				foreach (GodLesZ.Library.Logging.Config.AliasRepositoryAttribute configAttr in configAttributes) {
					try {
		...
		private const string DefaultRepositoryName = "GodLesZ.Library.Logging-default-repository";
