	...
	public class CompactRepositorySelector : IRepositorySelector {
		...
		public CompactRepositorySelector(Type defaultRepositoryType) {
			...
			if (!(typeof(ILoggerRepository).IsAssignableFrom(defaultRepositoryType))) {
				throw GodLesZ.Library.Logging.Util.SystemInfo.CreateArgumentOutOfRangeException("defaultRepositoryType", (object)defaultRepositoryType, "Parameter: defaultRepositoryType, Value: [" + defaultRepositoryType + "] out of range. Argument must implement the ILoggerRepository interface");
			}
		...
		/// returned. The default repository is named <c>GodLesZ.Library.Logging-default-repository</c>.
		/// </para>
		/// </remarks>
		...
		/// repository is <c>GodLesZ.Library.Logging-default-repository</c>. Other repositories 
