...
namespace GodLesZ.Library.Ragnarok.Grf {
	...
	public class RoGrfFileItem : ICloneable, IDisposable {
		protected string mNameHash = "";
		...
		public ERoGrfFileItemState State {
			get;
			internal set;
		}
		...
		public string NameHash {
			get {
				if (mNameHash == "")
					mNameHash = BuildNameHash(Filepath);
				return mNameHash;
			}
		}

		public bool IsAdded {
			get { return (State & ERoGrfFileItemState.Added) != 0; }
		}

		public bool IsUpdated {
			get { return (State & ERoGrfFileItemState.Updated) != 0; }
		}

		public bool IsDeleted {
			get { return (State & ERoGrfFileItemState.Deleted) != 0; }
		}
		...
		public RoGrfFileItem() {
			Cycle = -1;
			State = ERoGrfFileItemState.None;
		}
		...
		public void Flush(bool fromDispose) {
			...
			if (State == ERoGrfFileItemState.Updated && fromDispose == false) {
				return;
			}
			FileData = null;
		}
		...
		internal void WriteToFileTable(BinaryWriter tableWriter) {
			...
			tableWriter.Write((int)(DataOffset - RoGrfFile.GrfHeaderLen));
		}
