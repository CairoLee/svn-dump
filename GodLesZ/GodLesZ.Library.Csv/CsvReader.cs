...
using GodLesZ.Library.Csv.Events;
using GodLesZ.Library.Csv.Exceptions;
using Debug = System.Diagnostics.Debug;
using System.Globalization;
using System.IO;

using GodLesZ.Library.Csv.Resources;

namespace GodLesZ.Library.Csv {
	/// <summary>
	/// Represents a reader that provides fast, non-cached, forward-only access to CSV data.  
	/// </summary>
	public class CsvReader : IDataReader, IEnumerable<string[]>, IDisposable {
		...
		/// </exception>
		private void ValidateDataReader(EDataReaderValidations validations) {
			if ((validations & EDataReaderValidations.IsInitialized) != 0 && !_initialized)
				throw new InvalidOperationException(ExceptionMessage.NoCurrentRecord);

			if ((validations & EDataReaderValidations.IsNotClosed) != 0 && _isDisposed)
		...
		bool IDataReader.NextResult() {
			ValidateDataReader(EDataReaderValidations.IsNotClosed);
		...
		int IDataRecord.GetInt32(int i) {
			ValidateDataReader(EDataReaderValidations.IsInitialized | EDataReaderValidations.IsNotClosed);
