...
namespace GodLesZ.Library.ProcessEditAPI {
	...
	public static class PatternHelper {
		...
		public static uint FindPattern( IntPtr hProcess, uint dwStart, int nSize, byte[] bPattern, string szMask ) {
			...
			byte[] bData = MemoryHelper.ReadBytes( hProcess, dwStart, nSize );
			if( bData == null )
		...
		public static uint FindPattern( byte[] bData, byte[] bPattern, string szMask ) {
			...
			if( bPattern.Length > szMask.Length )
				szMask += new string( '?', bPattern.Length - szMask.Length );
			else if( bPattern.Length < szMask.Length )
				szMask = szMask.Substring( 0, bPattern.Length );
			...
			for( ix = 0; ix < DataLength; ix++ ) {
				...
				for( iy = 0; iy < PatternLength; iy++ ) {
					if( ( szMask[ iy ] == 'x' && bPattern[ iy ] != bData[ ix + iy ] ) || ( szMask[ iy ] == '!' && bPattern[ iy ] == bData[ ix + iy ] ) ) {
						bFound = false;
						break;
					}
				}
