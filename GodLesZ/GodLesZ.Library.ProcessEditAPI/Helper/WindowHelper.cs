	...
	public static class WindowHelper {
		...
		public static IntPtr[] FindWindows( string Classname, string WindowTitle ) {
			...
			lock( lWindowsLock ) {
				...
				foreach( IntPtr hWnd in lWindows )
					if( ( WindowTitle.Length > 0 && Imports.GetWindowTitle( hWnd ) == WindowTitle ) || ( Classname.Length > 0 && Imports.GetClassName( hWnd ) == Classname ) )
						hWnds.Add( hWnd );
			}
		...
		public static IntPtr[] FindMainWindows( string Classname, string WindowTitle ) {
			...
			foreach( Process proc in procs ) {
				if( proc.MainWindowHandle == IntPtr.Zero )
					continue;
				if( ( WindowTitle.Length > 0 && proc.MainWindowTitle == WindowTitle ) || ( Classname.Length > 0 && Imports.GetClassName( proc.MainWindowHandle ) == Classname ) )
					hWnds.Add( proc.MainWindowHandle );
			}
		...
		public static IntPtr[] FindWindowsContains( string Classname, string WindowTitle ) {
			...
			lock( lWindowsLock ) {
				...
				foreach( IntPtr hWnd in lWindows )
					if( ( WindowTitle.Length > 0 && Imports.GetWindowTitle( hWnd ).Contains( WindowTitle ) ) || ( Classname.Length > 0 && Imports.GetClassName( hWnd ).Contains( Classname ) ) )
						hWnds.Add( hWnd );
			}
		...
		public static IntPtr[] FindMainWindowsContains( string Classname, string WindowTitle ) {
			...
			foreach( Process proc in procs ) {
				if( proc.MainWindowHandle == IntPtr.Zero )
					continue;
				if( ( WindowTitle.Length > 0 && proc.MainWindowTitle.Contains( WindowTitle ) ) || ( Classname.Length > 0 && Imports.GetClassName( proc.MainWindowHandle ).Contains( Classname ) ) )
					hWnds.Add( proc.MainWindowHandle );
			}
