...
namespace GodLesZ.Library.ProcessEditAPI {
	...
	public sealed partial class ProcessEdit {
		...
		public uint Execute( uint dwStartAddress, uint dwParameter ) {
			...
			bSuccess = ( ThreadHelper.WaitForSingleObject( hThread, 10000 ) == WaitValues.WAIT_OBJECT_0 );
			if( bSuccess )
		...
		public IntPtr CreateRemoteThread( uint dwStartAddress, uint dwParameter, uint dwCreationFlags, out uint dwThreadId ) {
			if( mProcessOpen )
				return ThreadHelper.CreateRemoteThread( mProcess, dwStartAddress, dwParameter, dwCreationFlags, out dwThreadId );
		...
		public bool SuspendThread( IntPtr hThread ) {
			return ( ThreadHelper.SuspendThread( hThread ) == uint.MaxValue ) ? false : true;
		}
		...
		public bool SuspendThread() {
			return ( mThreadOpen ) ? this.SuspendThread( mThread ) : false;
		}
		...
		public bool ResumeThread( IntPtr hThread ) {
			return ( ThreadHelper.ResumeThread( hThread ) == uint.MaxValue ) ? false : true;
		}
		...
		public bool ResumeThread() {
			return ( mThreadOpen ) ? this.ResumeThread( mThread ) : false;
		}
