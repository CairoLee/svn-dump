...
namespace GodLesZ.Library.Xna.WindowLibrary.Controls {
	...
	public class Manager : DrawableGameComponent {
		...
		/// </summary>
		public virtual new GraphicsDevice GraphicsDevice {
		...
		public virtual ModalContainer ModalWindow {
			...
			internal set {
				...
				if (value != null) {
					value.ModalResult = EModalResult.None;
		...
		public virtual void Add(IEnumerable<Control> controls) {
			foreach (var con in controls) {
				Add(con);
			}
		}

		public virtual void Add(IEnumerable<Component> components) {
		...
		public virtual void Remove(IEnumerable<Component> components) {
			foreach (var con in components) {
				Remove(con);
			}
		}
		...
		private void MouseDownProcess(object sender, MouseEventArgs e) {
			...
			for (int i = c.Count - 1; i >= 0; i--) {
				if (CheckState(c[i]) && CheckPosition(c[i], e.Position)) {
					...
					c[i].SendMessage(EMessage.MouseDown, e);
		...
		private void MouseUpProcess(object sender, MouseEventArgs e) {
			...
			if (c != null) {
				if (CheckPosition(c, e.Position) && CheckOrder(c, e.Position) && states.Click == (int)e.Button && CheckButtons((int)e.Button)) {
					c.SendMessage(EMessage.Click, e);
				}
				states.Click = -1;
				c.SendMessage(EMessage.MouseUp, e);
		...
		private void MousePressProcess(object sender, MouseEventArgs e) {
			...
			if (c != null) {
				if (CheckPosition(c, e.Position)) {
					c.SendMessage(EMessage.MousePress, e);
				}
			}
		}
		...
		private void MouseMoveProcess(object sender, MouseEventArgs e) {
			...
			for (int i = c.Count - 1; i >= 0; i--) {
				...
				if (chsta && ((chpos && states.Over == c[i]) || (states.Buttons[(int)e.Button] == c[i]))) {
					c[i].SendMessage(EMessage.MouseMove, e);
					break;
				}
			}
			...
			for (int i = c.Count - 1; i >= 0; i--) {
				...
				if (chsta && !chpos && states.Over == c[i] && states.Buttons[(int)e.Button] == null) {
					states.Over = null;
					c[i].SendMessage(EMessage.MouseOut, e);
					break;
				}
			}
			...
			for (int i = c.Count - 1; i >= 0; i--) {
				...
				if (chsta && chpos && states.Over != c[i] && states.Buttons[(int)e.Button] == null) {
					if (states.Over != null) {
						states.Over.SendMessage(EMessage.MouseOut, e);
					}
					states.Over = c[i];
					c[i].SendMessage(EMessage.MouseOver, e);
					break;
				} else if (states.Over == c[i])
					break;
			}
		}
		...
		void GamePadDownProcess(object sender, GamePadEventArgs e) {
			...
			if (c != null && CheckState(c)) {
				...
				c.SendMessage(EMessage.GamePadDown, e);

				if (e.Button == c.GamePadActions.Click) {
					c.SendMessage(EMessage.Click, new MouseEventArgs(new MouseState(), EMouseButton.None, Point.Zero));
				}
			}
		}
		...
		void GamePadUpProcess(object sender, GamePadEventArgs e) {
			...
			if (c != null) {
				...
				states.Click = -1;
				states.Buttons[(int)e.Button] = null;
				c.SendMessage(EMessage.GamePadUp, e);
			}
		}
		...
		void GamePadPressProcess(object sender, GamePadEventArgs e) {
			Control c = states.Buttons[(int)e.Button];
			if (c != null) {
				c.SendMessage(EMessage.GamePadPress, e);
		...
		void KeyDownProcess(object sender, KeyEventArgs e) {
			...
			if (c != null && CheckState(c)) {
				if (states.Click == -1) {
					states.Click = (int)EMouseButton.None;
				}
				states.Buttons[(int)EMouseButton.None] = c;
				c.SendMessage(EMessage.KeyDown, e);
		...
		void KeyUpProcess(object sender, KeyEventArgs e) {
			Control c = states.Buttons[(int)EMouseButton.None];
			...
			if (c != null) {
				...
				states.Click = -1;
				states.Buttons[(int)EMouseButton.None] = null;
				c.SendMessage(EMessage.KeyUp, e);
			}
		}
		...
		void KeyPressProcess(object sender, KeyEventArgs e) {
			Control c = states.Buttons[(int)EMouseButton.None];
			if (c != null) {
				c.SendMessage(EMessage.KeyPress, e);
				...
				if ((e.Key == Microsoft.Xna.Framework.Input.Keys.Right ||
					 ...
					 e.Key == Microsoft.Xna.Framework.Input.Keys.Down) && !e.Handled && CheckButtons((int)EMouseButton.None)) {
					ProcessArrows(c, e, new GamePadEventArgs(PlayerIndex.One));
					KeyDownProcess(sender, e);
				} else if (e.Key == Microsoft.Xna.Framework.Input.Keys.Tab && !e.Shift && !e.Handled && CheckButtons((int)EMouseButton.None)) {
					TabNextControl(c);
					KeyDownProcess(sender, e);
				} else if (e.Key == Microsoft.Xna.Framework.Input.Keys.Tab && e.Shift && !e.Handled && CheckButtons((int)EMouseButton.None)) {
					TabPrevControl(c);
