...
namespace GodLesZ.Library.Xna.WindowLibrary.External.Zip {
	internal class ZipEntry {
		...
		private static bool ReadHeader(System.IO.Stream s, ZipEntry ze) {
			...
			ze._FileName = GodLesZ.Library.Xna.WindowLibrary.External.Zip.Shared.StringFromBuffer(block, 0, block.Length);

			ze._Extra = new byte[extraFieldLength];
			n = s.Read(ze._Extra, 0, ze._Extra.Length);

			// transform the time data into something usable
			ze._LastModified = GodLesZ.Library.Xna.WindowLibrary.External.Zip.Shared.PackedToDateTime(ze._LastModDateTime);

			// actually get the compressed size and CRC if necessary
			if ((ze._BitField & 0x0008) == 0x0008) {
				long posn = s.Position;
				long SizeOfDataRead = GodLesZ.Library.Xna.WindowLibrary.External.Zip.Shared.FindSignature(s, ZipEntryDataDescriptorSignature);
				if (SizeOfDataRead == -1)
					return false;
		...
		internal static ZipEntry Create(String filename) {
			...
			if (entry._LastModified.IsDaylightSavingTime()) {
				...
				entry._LastModDateTime = GodLesZ.Library.Xna.WindowLibrary.External.Zip.Shared.DateTimeToPacked(AdjustedTime);
			} else
				entry._LastModDateTime = GodLesZ.Library.Xna.WindowLibrary.External.Zip.Shared.DateTimeToPacked(entry._LastModified);
