...
namespace GodLesZ.Library.Xna.WindowLibrary.Controls {
	...
	public class TextBox : ClipControl {
		...
		private ETextBoxMode mode = ETextBoxMode.Normal;
		...
		private EScrollBars scrollBars = EScrollBars.Both;
		...
		public virtual EScrollBars ScrollBars {
		...
		public virtual ETextBoxMode Mode {
			get { return mode; }
			set {
				if (value != ETextBoxMode.Multiline) {
		...
		public override string Text {
			...
			set {
				if (mode != ETextBoxMode.Multiline && value != null) {
		...
		public TextBox(Manager manager)
			...
			vert = new ScrollBar(manager, EOrientation.Vertical);
			horz = new ScrollBar(manager, EOrientation.Horizontal);
		}
		...
		private void ProcessScrolling() {
			if (vert != null && horz != null) {
				...
				if (horz.PageSize > horz.Range)
					horz.PageSize = horz.Range;
		...
		private int CharAtPos(Point pos) {
			...
			string str = mode == ETextBoxMode.Multiline ? Lines[py] : shownText;
			...
			if (str != null && str != "") {
				...
				if (x > ClientLeft + ((int)font.MeasureString(str).X) - horz.Value - (font.MeasureString(str[str.Length - 1].ToString()).X / 3))
					px = str.Length;
			}
		...
		protected override void OnMouseUp(MouseEventArgs e) {
			...
			if (e.Button == EMouseButton.Left && !selection.IsEmpty && mode != ETextBoxMode.Password) {
		...
		private void SetupBars() {
			...
			if (vert != null) {
				...
				if (Height < 50 || (scrollBars != EScrollBars.Both && scrollBars != EScrollBars.Vertical))
					vert.Visible = false;
				else if ((scrollBars == EScrollBars.Vertical || scrollBars == EScrollBars.Both) && mode == ETextBoxMode.Multiline)
					vert.Visible = true;
			}
			if (horz != null) {
				horz.Left = 2;
				...
				if (Width < 50 || wordWrap || (scrollBars != EScrollBars.Both && scrollBars != EScrollBars.Horizontal))
					horz.Visible = false;
				else if ((scrollBars == EScrollBars.Horizontal || scrollBars == EScrollBars.Both) && mode == ETextBoxMode.Multiline && !wordWrap)
					horz.Visible = true;
			}
		...
		private List<string> SplitLines(string text) {
			if (buffer != text) {
				...
				if (posy < 0)
					posy = 0;
				if (posy > list.Count - 1)
					posy = list.Count - 1;

				if (posx < 0)
					posx = 0;
				if (posx > list[PosY].Length)
					posx = list[PosY].Length;
