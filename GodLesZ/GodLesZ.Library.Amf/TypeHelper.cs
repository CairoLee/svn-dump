...
namespace GodLesZ.Library.Amf {
	...
	public sealed class TypeHelper {
		...
		/// This method supports the infrastructure and is not intended to be used directly from your code.
		/// </summary>
		/// <returns>Lac locations used for assembly probing by GodLesZ.Library.Amf.</returns>
		public static string[] GetLacLocations() {
			...
			try {
				...
				log.Debug("Checking GodLesZ.Library.Amf location");
				try {
					// This is the GodLesZ.Library.Amf path
					...
					if (path != null) {
						Location location = new Location(path, "GodLesZ.Library.Amf location");
						locations.Add(location);
					}
				} catch (SecurityException) {
				}
			...
			try {
				if (isNullable) {
					...
					if (typeof(Guid) == targetType)
						return CanConvertToNullableGuid(obj);
				}
				...
				if (typeof(Guid) == targetType)
					return CanConvertToGuid(obj);
			} catch (InvalidCastException) {
			}

#if !SILVERLIGHT && !NET_2_0
			if (typeof(System.Xml.Linq.XDocument) == targetType && obj is XmlDocument)
				return true;
			if (typeof(System.Xml.Linq.XElement) == targetType && obj is XmlDocument)
				return true;
#endif
		...
		private static object ConvertChangeType(object value, Type targetType, bool isNullable) {
			...
			if (isNullable) {
				...
				if (typeof(Guid) == GetUnderlyingType(targetType))
...
#endif
#if !SILVERLIGHT && !NET_2_0
			if (typeof(System.Xml.Linq.XDocument) == targetType)
				return ConvertToXDocument(value);
			if (typeof(System.Xml.Linq.XElement) == targetType)
		...
		public static XDocument ConvertToXDocument(object value) {
			return
				...
				value == null ? _defaultXDocumentNullValue :
					GodLesZ.Library.Amf.Util.Convert.ToXDocument(value);
		}
		...
		public static XElement ConvertToXElement(object value) {
			return
				...
				value == null ? _defaultXElementNullValue :
					GodLesZ.Library.Amf.Util.Convert.ToXElement(value);
		}
		...
		public static ASObject ConvertDataTableToASO(DataTable dataTable, bool stronglyTyped) {
			...
			if (dataTable.ExtendedProperties["Service"] != null)
				...
				asObject["serviceName"] = "GodLesZ.Library.Amf.PageableResult";
			asObject["version"] = 1;
			asObject["cursor"] = 1;
