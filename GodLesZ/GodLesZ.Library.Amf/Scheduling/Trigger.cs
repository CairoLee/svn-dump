...
namespace GodLesZ.Library.Amf.Scheduling {
	public class Trigger : IComparable {
		...
		private int _misfireInstruction = GodLesZ.Library.Amf.Scheduling.MisfireInstruction.InstructionNotSet;
		...
		public void UpdateAfterMisfire() {
			...
			if (instr == GodLesZ.Library.Amf.Scheduling.MisfireInstruction.SmartPolicy) {
				if (RepeatCount == 0) {
					instr = GodLesZ.Library.Amf.Scheduling.MisfireInstruction.FireNow;
				} else if (RepeatCount == RepeatIndefinitely) {
					instr = GodLesZ.Library.Amf.Scheduling.MisfireInstruction.RescheduleNextWithRemainingCount;

				} else {
					instr = GodLesZ.Library.Amf.Scheduling.MisfireInstruction.RescheduleNowWithExistingRepeatCount;
				}
			} else if (instr == GodLesZ.Library.Amf.Scheduling.MisfireInstruction.FireNow && RepeatCount != 0) {
				instr = GodLesZ.Library.Amf.Scheduling.MisfireInstruction.RescheduleNowWithRemainingRepeatCount;
			}

			if (instr == GodLesZ.Library.Amf.Scheduling.MisfireInstruction.FireNow) {
				SetNextFireTime(DateTime.UtcNow);
			} else if (instr == GodLesZ.Library.Amf.Scheduling.MisfireInstruction.RescheduleNextWithExistingCount) {
				NullableDateTime newFireTime = GetFireTimeAfter(DateTime.UtcNow);
				SetNextFireTime(newFireTime);
			} else if (instr == GodLesZ.Library.Amf.Scheduling.MisfireInstruction.RescheduleNextWithRemainingCount) {
				NullableDateTime newFireTime = GetFireTimeAfter(DateTime.UtcNow);
				...
				SetNextFireTime(newFireTime);
			} else if (instr == GodLesZ.Library.Amf.Scheduling.MisfireInstruction.RescheduleNowWithExistingRepeatCount) {
			...
			} else if (instr == GodLesZ.Library.Amf.Scheduling.MisfireInstruction.RescheduleNowWithRemainingRepeatCount) {
				DateTime newFireTime = DateTime.UtcNow;
				int timesMissed = ComputeNumTimesFiredBetween(_nextFireTimeUtc, newFireTime);
