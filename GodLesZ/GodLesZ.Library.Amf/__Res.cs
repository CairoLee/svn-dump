...
namespace GodLesZ.Library.Amf {
	/// <summary>
	/// This type supports the infrastructure and is not intended to be used directly from the code.
	/// </summary>
	internal class __Res {
		...
		internal static string GetString(string key) {
			if (_resMgr == null) {
				_resMgr = new ResourceManager("GodLesZ.Library.Amf.Resources.Resource", typeof(__Res).Assembly);
			}
			string text = _resMgr.GetString(key);
			if (text == null) {
				throw new ApplicationException("Missing resource from GodLesZ.Library.Amf library!  Key: " + key);
			}
			return text;
		}
