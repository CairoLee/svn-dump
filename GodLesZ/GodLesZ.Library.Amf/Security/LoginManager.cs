	...
	class LoginManager {
		...
		public bool Logout() {
			...
			if (_loginCommand != null) {
				...
				if (log.IsErrorEnabled)
					log.Error(__Res.GetString(__Res.Security_LoginMissing));
				//GodLesZ.Library.Amf.Messaging.SecurityException se = new GodLesZ.Library.Amf.Messaging.SecurityException(NoLoginCommand, GodLesZ.Library.Amf.Messaging.SecurityException.ServerAuthorizationCode);
				//throw se;
				throw new UnauthorizedAccessException(__Res.GetString(__Res.Security_LoginMissing));
			}
			if (HttpContext.Current != null) {
