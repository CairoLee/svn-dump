...
using GodLesZ.Library.Amf.Browser;
...
namespace GodLesZ.Library.Amf {
	...
	public class FluorineGateway : IHttpModule, IRequiresSessionState, IRegisteredObject, IRequestHandlerHost {
				...
				lock (_objLock) {
					if (!_initialized) {
						...
						try {
							...
							WireAppDomain();
							RegisterObject();
						} catch (MethodAccessException) { } catch (SecurityException) { }

						FluorineWebContext.Initialize();

						Log.Info(__Res.GetString(__Res.ServiceBrowser_Aquire));
						try {
							Type type = ObjectFactory.Locate("GodLesZ.Library.Amf.ServiceBrowser.ServiceBrowserRenderer");
							if (type != null) {
		...
		private void RegisterObject() {
			...
			Log.Info("GodLesZ.Library.Amf registered in the hosting environment");
#endif
		}
