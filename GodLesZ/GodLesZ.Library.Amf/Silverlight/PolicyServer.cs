...
namespace GodLesZ.Library.Amf.Silverlight {
...
#endif
 {
	...
	public class PolicyServer {
		...
		public PolicyServer(string policyFile) {
			...
			try {
				if (log.IsDebugEnabled)
					log.Debug("Starting GodLesZ.Library.Amf Silverlight Policy Server");
				...
				if (log.IsDebugEnabled)
					log.Debug("Started GodLesZ.Library.Amf Silverlight Policy Server");
			} catch (Exception ex) {
				if (log.IsErrorEnabled)
					log.Error("Error starting GodLesZ.Library.Amf Silverlight Policy Server", ex);
			}
		}
		...
		public void Close() {
			try {
				if (_listener != null)
					_listener.Close();
				if (log.IsDebugEnabled)
					log.Debug("Stopped GodLesZ.Library.Amf Silverlight Policy Server");
			} catch (Exception ex) {
				if (log.IsErrorEnabled)
					log.Error("Error stopping GodLesZ.Library.Amf Silverlight Policy Server", ex);
			}
		}
