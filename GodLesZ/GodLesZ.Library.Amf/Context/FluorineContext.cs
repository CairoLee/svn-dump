...
namespace GodLesZ.Library.Amf.Context {
	/// <summary>
	/// Similary to the ASP.NET HttpContext class you can access the context for the current request from any code inside the same application domain.
	/// The context information is accessed through the static property Current on the FluorineContext class.
	/// </summary>
	/// <remarks>
	/// <para>
	/// For an AMF channel (Http request) the context wrapps the underlying HttpContext.
	/// </para>
	/// <para>
	/// The context is available only when client requests are handled (both HTTP and RTMP) and is not avaliable in a newly started thread.
	/// </para>
	/// <para>
	/// It is recommended to use FluorineContext instead of HttpContext if you do not want to tie your application to ASP.NET that would otherwise work without change with a RTMP channel (both APS.NET hosted or GodLesZ.Library.Amf Windows Service hosted).
	...
	public abstract class FluorineContext {
		...
		/// <summary>
		/// Return an <see cref="GodLesZ.Library.Amf.Context.IResource"/> handle for the specified location.
		/// </summary>
