...
namespace GodLesZ.Library.BlubbZip.Blubb {
	/// <summary>
	/// Basic implementation of <see cref="IBlubbZipEntryFactory"></see>
	/// </summary>
	public class BlubbZipEntryFactory : IBlubbZipEntryFactory {
		#region Enumerations
		/// <summary>
		/// Defines the possible values to be used for the <see cref="BlubbEntry.DateTime"/>.
		/// </summary>
		public enum TimeSetting {
			...
			/// specified via the <see cref="BlubbEntryFactory(DateTime)"/> constructor or 
			/// using the <see cref="BlubbEntryFactory(TimeSetting)"/> with the setting set
		...
		#region Constructors
		/// <summary>
		/// Initialise a new instance of the <see cref="BlubbEntryFactory"/> class.
		/// </summary>
		/// <remarks>A default <see cref="INameTransform"/>, and the LastWriteTime for files is used.</remarks>
		public BlubbZipEntryFactory() {
			nameTransform_ = new BlubbZipNameTransform();
		}

		/// <summary>
		/// Initialise a new instance of <see cref="BlubbEntryFactory"/> using the specified <see cref="TimeSetting"/>
		/// </summary>
		/// <param name="timeSetting">The <see cref="TimeSetting">time setting</see> to use when creating <see cref="BlubbEntry">Blubb entries</see>.</param>
		public BlubbZipEntryFactory( TimeSetting timeSetting ) {
			timeSetting_ = timeSetting;
			nameTransform_ = new BlubbZipNameTransform();
		}

		/// <summary>
		/// Initialise a new instance of <see cref="BlubbEntryFactory"/> using the specified <see cref="DateTime"/>
		/// </summary>
		/// <param name="time">The time to set all <see cref="BlubbEntry.DateTime"/> values to.</param>
		public BlubbZipEntryFactory( DateTime time ) {
		...
		#region Properties
		/// <summary>
		/// Get / set the <see cref="INameTransform"/> to be used when creating new <see cref="BlubbEntry"/> values.
		/// </summary>
		/// <remarks>
		/// Setting this property to null will cause a default <see cref="BlubbNameTransform">name transform</see> to be used.
		/// </remarks>
		...
		/// <summary>
		/// Make a new <see cref="BlubbEntry"/> for a file.
		/// </summary>
		/// <param name="fileName">The name of the file to create a new entry for.</param>
		/// <returns>Returns a new <see cref="BlubbEntry"/> based on the <paramref name="fileName"/>.</returns>
		public BlubbZipEntry MakeFileEntry( string fileName ) {
			return MakeFileEntry( fileName, true );
		}

		/// <summary>
		/// Make a new <see cref="BlubbEntry"/> from a name.
		/// </summary>
		/// <param name="fileName">The name of the file to create a new entry for.</param>
		/// <param name="useFileSystem">If true entry detail is retrieved from the file system if the file exists.</param>
		/// <returns>Returns a new <see cref="BlubbEntry"/> based on the <paramref name="fileName"/>.</returns>
		public BlubbZipEntry MakeFileEntry( string fileName, bool useFileSystem ) {
			BlubbZipEntry result = new BlubbZipEntry( nameTransform_.TransformFile( fileName ) );
			...
			if( fi != null && fi.Exists ) {
				switch( timeSetting_ ) {
					...
					default:
						throw new BlubbZipException( "Unhandled time setting in MakeFileEntry" );
				}
		...
		/// <summary>
		/// Make a new <see cref="BlubbEntry"></see> for a directory.
		/// </summary>
		/// <param name="directoryName">The raw untransformed name for the new directory</param>
		/// <returns>Returns a new <see cref="BlubbEntry"></see> representing a directory.</returns>
		public BlubbZipEntry MakeDirectoryEntry( string directoryName ) {
		...
		/// <param name="useFileSystem">If true entry detail is retrieved from the file system if the file exists.</param>
		/// <returns>Returns a new <see cref="BlubbEntry"></see> representing a directory.</returns>
		public BlubbZipEntry MakeDirectoryEntry( string directoryName, bool useFileSystem ) {

			BlubbZipEntry result = new BlubbZipEntry( nameTransform_.TransformDirectory( directoryName ) );
			result.Size = 0;
			...
			if( di != null && di.Exists ) {
				switch( timeSetting_ ) {
					...
					default:
						throw new BlubbZipException( "Unhandled time setting in MakeDirectoryEntry" );
				}
