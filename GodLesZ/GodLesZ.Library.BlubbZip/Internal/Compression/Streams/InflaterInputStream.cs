...
namespace GodLesZ.Library.BlubbZip.Blubb.Compression.Streams {
	...
	public class InflaterInputBuffer {
		...
		public void Fill() {
			...
			while( toRead > 0 ) {
				...
				if( count <= 0 ) {
					if( rawLength == 0 ) {
						throw new BlubbZipBaseException( "Unexpected EOF" );
					}
					break;
				}
				rawLength += count;
...
#endif
 {
		...
		public int ReadLeByte() {
			if( available <= 0 ) {
				Fill();
				if( available <= 0 ) {
					throw new BlubbZipException( "EOF in header" );
				}
			}
	...
	/// as the <see cref="GodLesZ.Library.BlubbZip.GBlubb.GBlubbInputStream">GBlubbInputStream</see>.
	///
	...
	public class InflaterInputStream : Stream {
		...
		public override int Read( byte[] buffer, int offset, int count ) {
			if( inf.IsNeedingDictionary ) {
				throw new BlubbZipBaseException( "Need a dictionary" );
			}
			...
			while( true ) {
				...
				if( inf.IsNeedingInput ) {
					...
					throw new BlubbZipException( "Dont know what to do" );
				}
			}
