...
namespace GodLesZ.Library.BlubbZip.Blubb {
	/// <summary>
	/// SimpleBlubbEvents supports all events applicable to <see cref="SimpleBlubb">SimpleBlubb</see> operations.
	/// </summary>
	public class SimpleBlubbEvents {
		/// <summary>
	...
	/// <summary>
	/// SimpleBlubb provides facilities for creating and extracting blubb files.
	/// </summary>
	public class SimpleBlubbZip {
		#region Enumerations
		/// <summary>
		...
		#region Constructors
		/// <summary>
		/// Initialise a default instance of <see cref="SimpleBlubb"/>.
		/// </summary>
		public SimpleBlubbZip() {
		}

		/// <summary>
		/// Initialise a new instance of <see cref="SimpleBlubb"/>
		/// </summary>
		/// <param name="events">The <see cref="SimpleBlubbEvents">events</see> to use during operations.</param>
		public SimpleBlubbZip( SimpleBlubbEvents events ) {
			events_ = events;
		}
		#endregion
		...
		/// <summary>
		/// Get or set the <see cref="INameTransform"></see> active when creating Blubb files.
		/// </summary>
		...
		/// <summary>
		/// Get or set the <see cref="IEntryFactory"></see> active when creating Blubb files.
		/// </summary>
		public IBlubbZipEntryFactory EntryFactory {
			...
			set {
				if( value == null ) {
					entryFactory_ = new BlubbZipEntryFactory();
				} else {
					entryFactory_ = value;
				}
			}
		...
		/// <summary>
		/// Gets or sets the setting for <see cref="UseBlubb64">Blubb64 handling when writing.</see>
		/// </summary>
		/// <remarks>
		...
		/// read Blubb64 archives. However it does avoid the situation were a large file
		/// is added and cannot be completed correctly.
		/// NOTE: Setting the size for entries before they are added is the best solution!
		/// By default the EntryFactory used by SimpleBlubb will set fhe file size.
		/// </remarks>
		...
		/// Create a blubb file.
		/// </summary>
		/// <param name="blubbFileName">The name of the blubb file to create.</param>
		/// <param name="sourceDirectory">The directory to source files from.</param>
		/// <param name="recurse">True to recurse directories, false for no recursion.</param>
		/// <param name="fileFilter">The <see cref="PathFilter">file filter</see> to apply.</param>
		/// <param name="directoryFilter">The <see cref="PathFilter">directory filter</see> to apply.</param>
		public void CreateBlubb( string blubbFileName, string sourceDirectory,
			bool recurse, string fileFilter, string directoryFilter ) {
			CreateBlubb( File.Create( blubbFileName ), sourceDirectory, recurse, fileFilter, directoryFilter );
		}

		/// <summary>
		/// Create a blubb file/archive.
		/// </summary>
		/// <param name="blubbFileName">The name of the blubb file to create.</param>
		/// <param name="sourceDirectory">The directory to obtain files and directories from.</param>
		/// <param name="recurse">True to recurse directories, false for no recursion.</param>
		/// <param name="fileFilter">The file filter to apply.</param>
		public void CreateBlubb( string blubbFileName, string sourceDirectory, bool recurse, string fileFilter ) {
			CreateBlubb( File.Create( blubbFileName ), sourceDirectory, recurse, fileFilter, null );
		}
		...
		public void ExtractBlubb( string blubbFileName, string targetDirectory, string fileFilter ) {
			ExtractBlubb( blubbFileName, targetDirectory, Overwrite.Always, null, fileFilter, null, restoreDateTimeOnExtract_ );
		}
			...
			if( !e.HasMatchingFiles && CreateEmptyDirectories ) {
				...
				if( e.ContinueRunning ) {
					if( e.Name != sourceDirectory_ ) {
						BlubbZipEntry entry = entryFactory_.MakeDirectoryEntry( e.Name );
		...
		void ProcessFile( object sender, ScanEventArgs e ) {
			...
			if( e.ContinueRunning ) {
				using( FileStream stream = File.OpenRead( e.Name ) ) {
					BlubbZipEntry entry = entryFactory_.MakeFileEntry( e.Name );
					outputStream_.PutNextEntry( entry );
		...
		void ExtractFileEntry( BlubbZipEntry entry, string targetName ) {
			bool proceed = true;
				...
				if( continueRunning_ ) {
					try {
						using( FileStream outputStream = File.Create( targetName ) ) {
							...
							if( ( events_ != null ) && ( events_.Progress != null ) ) {
								StreamUtils.Copy( blubbFile_.GetInputStream( entry ), outputStream, buffer_,
						...
						if( RestoreAttributesOnExtract && entry.IsDOSEntry && ( entry.ExternalFileAttributes != -1 ) ) {
							...
							// TODO: SimpleBlubb - Setting of other file attributes on extraction is a little trickier.
		...
		void ExtractEntry( BlubbZipEntry entry ) {
		...
		byte[] buffer_;
		BlubbZipOutputStream outputStream_;
		...
		SimpleBlubbEvents events_;
		IBlubbZipEntryFactory entryFactory_ = new BlubbZipEntryFactory();
