...
namespace GodLesZ.Library.BlubbZip.Blubb {
	...
	/// <summary>
	/// This class assists with writing/reading from Blubb files.
	/// </summary>
	internal class BlubbZipHelperStream : Stream {
		#region Constructors
		/// <summary>
		/// Initialise an instance of this class.
		/// </summary>
		/// <param name="name">The name of the file to open.</param>
		public BlubbZipHelperStream( string name ) {
			stream_ = new FileStream( name, FileMode.Open, FileAccess.ReadWrite );
			isOwner_ = true;
		}

		/// <summary>
		/// Initialise a new instance of <see cref="BlubbHelperStream"/>.
		/// </summary>
		/// <param name="stream">The stream to use.</param>
		public BlubbZipHelperStream( Stream stream ) {
			stream_ = stream;
		}
		#endregion
		...
		/// <summary>
		/// Write Blubb64 end of central directory records (File header and locator).
		/// </summary>
		/// <param name="noOfEntries">The number of entries in the central directory.</param>
		/// <param name="sizeEntries">The size of entries in the central directory.</param>
		/// <param name="centralDirOffset">The offset of the dentral directory.</param>
		public void WriteBlubb64EndOfCentralDirectory( long noOfEntries, long sizeEntries, long centralDirOffset ) {
			long centralSignatureOffset = stream_.Position;
			WriteLEInt( BlubbZipConstants.Blubb64CentralFileHeaderSignature );
			WriteLELong( 44 );    // Size of this record (total size of remaining fields in header or full size - 12)
			WriteLEShort( BlubbZipConstants.VersionMadeBy );   // Version made by
			WriteLEShort( BlubbZipConstants.VersionBlubb64 );   // Version to extract
			...
			// blubb64 extensible data sector not catered for here (variable size)

			// Write the Blubb64 end of central directory locator
			WriteLEInt( BlubbZipConstants.Blubb64CentralDirLocatorSignature );

			// no of the disk with the start of the blubb64 end of central directory
			WriteLEInt( 0 );

			// relative offset of the blubb64 end of central directory record
		...
		public void WriteEndOfCentralDirectory( long noOfEntries, long sizeEntries,
			...
			if( ( noOfEntries >= 0xffff ) ||
				...
				WriteBlubb64EndOfCentralDirectory( noOfEntries, sizeEntries, startOfCentralDirectory );
			}

			WriteLEInt( BlubbZipConstants.EndOfCentralDirectorySignature );

			// TODO: BlubbFile Multi disk handling not done
			...
			// Number of entries
			if( noOfEntries >= 0xffff ) {
				WriteLEUshort( 0xffff );  // Blubb64 marker
			...
			if( sizeEntries >= 0xffffffff ) {
				WriteLEUint( 0xffffffff );    // Blubb64 marker
			} else {
			...
			if( commentLength > 0xffff ) {
				throw new BlubbZipException( string.Format( "Comment length({0}) is too long can only be 64K", commentLength ) );
			}
		...
		public int WriteDataDescriptor( BlubbZipEntry entry ) {
			if( entry == null ) {
