...
namespace GodLesZ.Library.BlubbZip.Blubb {
	...
	/// <summary>
	/// This class represents an entry in a blubb archive.  This can be a file
	/// or a directory
	/// BlubbFile and BlubbInputStream will give you instances of this class as 
	/// information about the members in an archive.  BlubbOutputStream
	/// uses an instance of this class when creating an entry in a Blubb file.
	/// <br/>
	/// <br/>Author of the original java version : Jochen Hoenicke
	/// </summary>
	public class BlubbZipEntry : ICloneable {
		[Flags]
		enum Known : byte {
			None = 0,
			Size = 0x01,
		...
		#region Constructors
		/// <summary>
		/// Creates a blubb entry with the given name.
		/// </summary>
		/// <param name="name">
		...
		/// </exception>
		public BlubbZipEntry( string name )
			: this( name, 0, BlubbZipConstants.VersionMadeBy, CompressionMethod.Deflated ) {
		}

		/// <summary>
		/// Creates a blubb entry with the given name and version required to extract
		/// </summary>
		/// <param name="name">
		/// The name for this entry. Can include directory components.
		...
		/// The name passed is null
		/// </exception>
		internal BlubbZipEntry( string name, int versionRequiredToExtract )
			: this( name, versionRequiredToExtract, BlubbZipConstants.VersionMadeBy,
		...
		/// <param name="versionRequiredToExtract">Minimum required blubb feature version required to extract this entry</param>
		...
		/// </exception>
		/// <remarks>
		/// This constructor is used by the BlubbFile class when reading from the central header
		/// It is not generally useful, use the constructor specifying the name only.
		/// </remarks>
		internal BlubbZipEntry( string name, int versionRequiredToExtract, int madeByInfo,
		...
		/// <summary>
		/// Creates a deep copy of the given blubb entry.
		/// </summary>
		/// <param name="entry">
		/// The entry to copy.
		/// </param>
		[Obsolete( "Use Clone instead" )]
		public BlubbZipEntry( BlubbZipEntry entry ) {
			if( entry == null )
			...
			blubbFileIndex = entry.blubbFileIndex;
			offset = entry.offset;

			forceBlubb64_ = entry.forceBlubb64_;
		...
		/// and uncompressed size are were not able to be written during blubb file creation
		...
		/// <summary>
		/// Get/Set index of this entry in Blubb file
		/// </summary>
		/// <remarks>This is only valid when the entry is part of a <see cref="BlubbFile"></see></remarks>
		public long BlubbFileIndex {
			get { return blubbFileIndex; }
			set { blubbFileIndex = value; }
		}
		...
		public bool IsDOSEntry {
			get { return ( ( HostSystem == (int)HostSystemID.Msdos ) || ( HostSystem == (int)HostSystemID.WindowsNT ) ); }
		}

		/// <summary>
		/// Test the external attributes for this <see cref="BlubbEntry"/> to
		...
		/// 		
		/// <remarks>
		/// The values for this as defined in the Blubb File format and by others are shown below.  The values are somewhat
		/// misleading in some cases as they are not all used as shown.  You should consult the relevant documentation
		/// to obtain up to date and correct information.  The modified appnote by the infoblubb group is
		...
		/// <summary>
		/// Get minimum Blubb feature version required to extract this entry
		/// </summary>		
		/// <remarks>
		...
		/// 4.5 - File uses Blubb64 format extensions<br/>
		...
		public int Version {
			get {
				...
				if( versionToExtract != 0 ) {
					...
					int result = 10;
					if( CentralHeaderRequiresBlubb64 ) {
						result = BlubbZipConstants.VersionBlubb64;
		...
		public bool CanDecompress {
			get { return Version <= BlubbZipConstants.VersionMadeBy && ( Version == 10 || Version == 11 || Version == 20 || Version == 45 ) && IsCompressionMethodSupported(); }
		}

		/// <summary>
		/// Force this entry to be recorded using Blubb64 extensions.
		/// </summary>
		public void ForceBlubb64() {
			forceBlubb64_ = true;
		}

		/// <summary>
		/// Get a value indicating wether Blubb64 extensions were forced.
		/// </summary>
		/// <returns>A <see cref="bool"/> value of true if Blubb64 extensions have been forced on; false if not.</returns>
		public bool IsBlubb64Forced() {
			return forceBlubb64_;
		}

		/// <summary>
		/// Gets a value indicating if the entry requires Blubb64 extensions 
		/// to store the full entry values.
		/// </summary>
		/// <value>A <see cref="bool"/> value of true if a local header requires Blubb64 extensions; false if not.</value>
		public bool LocalHeaderRequiresBlubb64 {
			get {
				bool result = forceBlubb64_;

				if( !result ) {
					ulong trueCompressedSize = compressedSize;

					if( ( versionToExtract == 0 ) && IsCrypted ) {
						trueCompressedSize += BlubbZipConstants.CryptoHeaderSize;
					}

					// TODO: A better estimation of the true limit based on compression overhead should be used
					// to determine when an entry should use Blubb64.
					result =
						( ( this.size >= uint.MaxValue ) || ( trueCompressedSize >= uint.MaxValue ) ) &&
						( ( versionToExtract == 0 ) || ( versionToExtract >= BlubbZipConstants.VersionBlubb64 ) );
				}
		...
		/// <summary>
		/// Get a value indicating wether the central directory entry requires Blubb64 extensions to be stored.
		/// </summary>
		public bool CentralHeaderRequiresBlubb64 {
			get { return LocalHeaderRequiresBlubb64 || ( offset >= uint.MaxValue ); }
		}
		...
		/// See the <see cref="BlubbNameTransform"/> class, or <see cref="CleanName(string)"/>
		///</remarks>
		public string Name {
			get { return name; }
		}
		...
		/// avoid compatability problems with some archivers which dont understand Blubb64 extensions.</remarks>
		public long Size {
		...
		/// </returns>
		/// <see cref="GodLesZ.Library.BlubbZip.Blubb.CompressionMethod.Deflated"/>
		/// <see cref="GodLesZ.Library.BlubbZip.Blubb.CompressionMethod.Stored"/>
		...
		/// </returns>
		/// <remarks>
		/// A comment is only available for entries when read via the <see cref="BlubbFile"/> class.
		/// The <see cref="BlubbInputStream"/> class doesnt have the comment data available.
		/// </remarks>
		...
		public bool IsDirectory {
			get {
				...
				bool result = ( nameLength > 0 && ( name[ nameLength - 1 ] == '/' || name[ nameLength - 1 ] == '\\' ) ) || HasDosAttributes( 16 );
				return result;
			}
		}
		...
		/// <summary>
		/// Creates a copy of this blubb entry.
		/// </summary>
		/// <returns>An <see cref="Object"/> that is a copy of the current instance.</returns>
		public object Clone() {
			BlubbZipEntry result = (BlubbZipEntry)this.MemberwiseClone();
		...
		/// <summary>
		/// Gets a string representation of this BlubbEntry.
		/// </summary>
		/// <returns>A readable textual representation of this <see cref="BlubbEntry"/></returns>
		...
		/// <summary>
		/// Cleans a name making it conform to Blubb file conventions.
		/// Devices names ('c:\') and UNC share names ('\\server\share') are removed
		/// and forward slashes ('\') are converted to back slashes ('/').
		/// Names are made relative by trimming leading slashes which is compatible
		/// with the BlubbZip naming convention.
		/// </summary>
		/// <param name="name">The name to clean</param>
		/// <returns>The 'cleaned' name.</returns>
		/// <remarks>
		/// The <seealso cref="BlubbNameTransform">Blubb name transform</seealso> class is more flexible.
		/// </remarks>
		...
		private long blubbFileIndex = -1;
		private long offset;

		private bool forceBlubb64_;
