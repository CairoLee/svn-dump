...
namespace GodLesZ.Library.BlubbZip.Blubb {
	...
	/// 
	/// <summary>
	/// A class to handle the extra data field for Blubb entries
	/// </summary>
	/// <remarks>
	...
	/// </remarks>
	sealed public class BlubbZipExtraData : IDisposable {
		#region Constructors
		/// <summary>
		/// Initialise a default instance.
		/// </summary>
		public BlubbZipExtraData() {
			Clear();
		}
		...
		public BlubbZipExtraData( byte[] data ) {
			if( data == null ) {
				data_ = new byte[ 0 ];
			} else {
				data_ = data;
		...
		public byte[] GetEntryData() {
			if( Length > ushort.MaxValue ) {
				throw new BlubbZipException( "Data exceeds maximum length" );
			}
		...
		public int UnreadCount {
			get {
				if( ( readValueStart_ > data_.Length ) ||
					( readValueStart_ < 4 ) ) {
					throw new BlubbZipException( "Find must be called before calling a Read method" );
				}
		...
		void ReadCheck( int length ) {
			...
			if( index_ > readValueStart_ + readValueLength_ - length ) {
				throw new BlubbZipException( "End of extra data" );
			}
		}
