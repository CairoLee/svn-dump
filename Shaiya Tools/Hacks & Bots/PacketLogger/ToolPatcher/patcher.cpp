...
int _tmain( int argc, TCHAR** argv ){
	...
	printf( "GodLesZ's Exe Patcher Tool\n" );
	if( argc < 3 ){
		_tprintf( _T( "Usage: %s <exe path> <dll path>\n" ), argv[ 0 ] );
		return 0;
	}
	...
	if( CreateExeWithDllImport( exepath, dllpath ) == false ){
		_tprintf( _T( "failed to patch the File..\n" ) );
		return 0;
	}
	
	_tprintf( _T( "Patch successfull!\n" ) );
}
