...
namespace MarketTool.Library {
	...
	public class ImageComboBox : ComboBox {
		...
		public void LoadList( ImageList ImageList, EImageComboItemTextAlign TextAlign ) {
			this.ImageList = ImageList;

			Items.Clear();
			foreach( string key in ImageList.Images.Keys ) {
				Items.Add( new ImageComboItem( key, this.Font, this.ForeColor, ImageList.Images.IndexOfKey( key ), TextAlign ) );
			}
			SelectedIndex = 0;
		}
