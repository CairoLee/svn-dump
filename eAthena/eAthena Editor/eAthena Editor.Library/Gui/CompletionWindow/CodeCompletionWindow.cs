...
namespace GodLesZ.eAthenaEditor.Library.Gui.CompletionWindow {
	public class CodeCompletionWindow : AbstractCompletionWindow {
		...
		CodeCompletionWindow(ICompletionDataProvider completionDataProvider, ICompletionData[] completionData, Form parentForm, TextEditorControl control, bool showDeclarationWindow, bool fixedListViewWidth)
			...
			this.mDrawingSize = GetListViewSize();
			SetLocation();
		...
		protected override void CaretOffsetChanged(object sender, EventArgs e) {
			int offset = mEditorControl.ActiveTextAreaControl.Caret.Offset;
			...
			if (offset < startOffset || offset > endOffset) {
				...
				codeCompletionListView.SelectItemWithStart(mEditorControl.Document.GetText(startOffset, offset - startOffset));
			}
		}
		...
		bool InsertSelectedItem(char ch) {
			...
			bool result = false;
			if (data != null) {
				mEditorControl.BeginUpdate();

				try {
					if (endOffset - startOffset > 0) {
						mEditorControl.Document.Remove(startOffset, endOffset - startOffset);
					}
					Debug.Assert(startOffset <= document.TextLength);
					result = dataProvider.InsertAction(data, mEditorControl.ActiveTextAreaControl.TextArea, startOffset, ch);
				} finally {
					mEditorControl.EndUpdate();
				}
			}
			Close();
			return result;
