...
using GodLesZ.eAthenaEditor.Library.Actions;
...
namespace GodLesZ.eAthenaEditor.Library {
	...
	public class TextArea : Control {
		...
		public static DeclarationViewWindow ToolTipWnd;
		static string oldToolTip;

		void SetToolTip(string text, int lineNumber) {
			if (ToolTipWnd == null || ToolTipWnd.IsDisposed)
				ToolTipWnd = new DeclarationViewWindow(this.FindForm());
			if (oldToolTip == text)
				return;
			if (text == null) {
				ToolTipWnd.Hide();
				...
				p.Offset(3, 3);
				ToolTipWnd.Location = p;
				ToolTipWnd.Description = text;
				ToolTipWnd.HideOnClick = true;
				ToolTipWnd.Show();
			}
			oldToolTip = text;
		}
