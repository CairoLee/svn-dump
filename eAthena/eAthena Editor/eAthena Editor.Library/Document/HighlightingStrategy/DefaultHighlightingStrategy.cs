...
namespace GodLesZ.eAthenaEditor.Library.Document {
	...
	public class DefaultHighlightingStrategy : IHighlightingStrategyUsingRuleSets {
		...
		private List<HighlightRuleSet> mRules = new List<HighlightRuleSet>();

		private Dictionary<string, HighlightColor> mEnvironmentColors = new Dictionary<string, HighlightColor>();
		private Dictionary<string, string> mProperties = new Dictionary<string, string>();
		private string[] extensions;

		private HighlightColor mDigitColor;
		private HighlightRuleSet mDefaultRuleSet = null;

		public HighlightColor DigitColor {
			get { return mDigitColor; }
			set { mDigitColor = value; }
		}

		public IEnumerable<KeyValuePair<string, HighlightColor>> EnvironmentColors {
			get { return mEnvironmentColors; }
		}
		...
		protected void ImportSettingsFrom(DefaultHighlightingStrategy source) {
			...
			mDigitColor = source.mDigitColor;
			mDefaultRuleSet = source.mDefaultRuleSet;
			mName = source.mName;
			mRules = source.mRules;
			mEnvironmentColors = source.mEnvironmentColors;
		...
		public HighlightRuleSet FindHighlightRuleSet(string name) {
			foreach (HighlightRuleSet ruleSet in mRules) {
		...
		public void SetColorFor(string name, HighlightColor color) {
			...
			mEnvironmentColors[name] = color;
		}

		public HighlightColor GetColorFor(string name) {
			HighlightColor color;
			if (mEnvironmentColors.TryGetValue(name, out color))
				return color;
			else
				return defaultTextColor;
		}
