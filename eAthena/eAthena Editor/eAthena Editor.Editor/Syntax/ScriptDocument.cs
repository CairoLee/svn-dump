...
using GodLesZ.eAthenaEditor.Library.Undo;

namespace GodLesZ.eAthenaEditor.Editor.Syntax {

	public class ScriptDocument : IDocument {
		bool readOnly = false;
		...
		public ScriptEntityManager FunctionMarkerManager {
			get;
			set;
		}
		...
		public void Insert(int offset, string text) {
			...
			if (text == "}" && offset > 0) {
				char chBefore = GetCharAt(offset - 1);
				if (chBefore == '\t') {
					Remove(offset - 1, 1);
					offset--;
				}
