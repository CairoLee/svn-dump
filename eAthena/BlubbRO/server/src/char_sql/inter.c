...
int inter_accreg_tosql(int account_id, int char_id, struct accreg* reg, int type)
{
	...
	case 3: //Char Reg
		...
		break;
	case 2: //Account Reg
		if( SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `account_id`='%d'", reg_db_login, account_id) )
			Sql_ShowDebug(sql_handle);
		char_id = 0;
		break;
	case 1: //Account2 Reg
	...
	switch(type) {
		case 3:
			if (SQL_ERROR == SqlStmt_Prepare(stmt, "INSERT INTO `%s` (`char_id`, `str`, `value`) VALUES ('%d',?,?)", reg_db_char, char_id)) {
				SqlStmt_ShowDebug(stmt);
			}
			break;
		case 2:
			if (SQL_ERROR == SqlStmt_Prepare(stmt, "INSERT INTO `%s` (`account_id`, `str`, `value`) VALUES ('%d',?,?)", reg_db_login, account_id)) {
				SqlStmt_ShowDebug(stmt);
			}
			break;	
	}
...
int inter_accreg_fromsql(int account_id,int char_id, struct accreg *reg, int type)
{
	...
	{
	case 3: //char reg
		if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `str`, `value` FROM `%s` WHERE `char_id`='%d'", reg_db_char, char_id) )
			Sql_ShowDebug(sql_handle);
		break;
	case 2: //account reg
		if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `str`, `value` FROM `%s` WHERE `account_id`='%d'", reg_db_login, account_id) )
			Sql_ShowDebug(sql_handle);
		break;
	case 1: //account2 reg
