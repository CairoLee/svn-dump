...
char reg_db_char[256] = "char_reg";
char reg_db_login[256] = "login_reg";
...
{	//Insert the barebones to then update the rest.
	...
	// entdeckte Dungeons
	diff = 0;
	for( i = 0; i < MAX_DUNGEON; i++ )
		if( cp->dungeon[i] == false && p->dungeon[i] == true ) {
			if( SQL_ERROR == Sql_Query( sql_handle, "INSERT DELAYED INTO `char_dungeon` VALUES ( '%d', '%d' )", p->account_id, i ) )
				Sql_ShowDebug( sql_handle );
			diff = 1;
		}
	if( diff ) {
		strcat( save_status, " dungeons" );
		diff = 0;
	}
	...
	for( i = 0; i < MAX_REGION; i++ )
		if( cp->region[i] == false && p->region[i] == true ) {
			if( SQL_ERROR == Sql_Query( sql_handle, "INSERT DELAYED INTO `char_region` VALUES ( '%d', '%d' )", p->account_id, i ) )
				Sql_ShowDebug( sql_handle );
			diff = 1;
		}
	if( diff ) {
		strcat( save_status, " regionen" );
		diff = 0;
	}

	// Achievements
	for (i = 0; i < MAX_ACHIEVEMENTS; i++){
		if (cp->achieveData[i].update == false && p->achieveData[i].update == true){
			for (j = 0; j < MAX_ACHIEVE_PARAMS; j++) {
				if (SQL_ERROR == Sql_Query(sql_handle, 
					"REPLACE INTO `char_achievements` ( `AchieveIndex`, `AchieveRequireIndex`, `AchieveID`, `CharID`, `AchieveCounter`, `Finished` ) VALUES ( '%d', '%d', '%d', '%d', '%d' )", 
					i, j, p->achieveData[i].AchieveID, p->char_id, p->achieveData[ i ].count[j], (p->achieveData[ i ].finished ? 1 : 0))
				) {
					Sql_ShowDebug( sql_handle );
				}
			}
...
int mmo_char_fromsql(int char_id, struct mmo_charstatus* p, bool load_everything)
{
	...
	/* entdeckte Dungeons */
	if( SQL_ERROR == SqlStmt_Prepare( stmt, "SELECT `DungeonID` FROM `char_dungeon` WHERE `AccountID` = ?" )
	||	SQL_ERROR == SqlStmt_BindParam( stmt, 0, SQLDT_INT, &p->account_id, 0 )
	||	SQL_ERROR == SqlStmt_Execute( stmt )
	||	SQL_ERROR == SqlStmt_BindColumn( stmt, 0, SQLDT_INT,  &index, 0, NULL, NULL ) )
		SqlStmt_ShowDebug( stmt );

	while( SQL_SUCCESS == SqlStmt_NextRow( stmt ) ) {
		if( index < 1 || index > MAX_DUNGEON )
			ShowWarning( "mmo_char_fromsql: invalid dungeon index ( %d ) of character %s ( AID=%d,CID=%d )\n", index, p->name, p->account_id, p->char_id );
		else
			p->dungeon[index] = true;
	}
	strcat( t_msg, " dungeons" );


	/* entdeckte Regionen */
	if( SQL_ERROR == SqlStmt_Prepare( stmt, "SELECT `RegionID` FROM `char_region` WHERE `AccountID` = ?" )
	||	SQL_ERROR == SqlStmt_BindParam( stmt, 0, SQLDT_INT, &p->account_id, 0 )
	||	SQL_ERROR == SqlStmt_Execute( stmt )
	||	SQL_ERROR == SqlStmt_BindColumn( stmt, 0, SQLDT_INT,  &index, 0, NULL, NULL ) )
		SqlStmt_ShowDebug( stmt );
	...
	while( SQL_SUCCESS == SqlStmt_NextRow( stmt ) ) {
		if( index < 1 || index > MAX_REGION )
			ShowWarning( "mmo_char_fromsql: invalid region index ( %d ) of character %s ( AID=%d,CID=%d )\n", index, p->name, p->account_id, p->char_id );
		else
			p->region[index] = true;
	}
	strcat( t_msg, " regionen" );


	/* Achievemen Data */
	{
		int achieveIndex = 0, achieveRequireIndex = 0, achieveID = 0, achieveCounter = 0, finished = 0;
		if( SQL_ERROR == SqlStmt_Prepare( stmt, "SELECT `AchieveIndex`, `AchieveRequireIndex`, `AchieveID`, `AchieveCounter`, `Finished` FROM `char_achievements` WHERE `CharID` = ?" )
		||	SQL_ERROR == SqlStmt_BindParam( stmt, 0, SQLDT_INT, &p->char_id, 0 )
		||	SQL_ERROR == SqlStmt_Execute( stmt )
		||	SQL_ERROR == SqlStmt_BindColumn( stmt, 0, SQLDT_INT, &achieveIndex, 0, NULL, NULL )
		||	SQL_ERROR == SqlStmt_BindColumn( stmt, 1, SQLDT_INT, &achieveRequireIndex, 0, NULL, NULL )
		||	SQL_ERROR == SqlStmt_BindColumn( stmt, 2, SQLDT_INT, &achieveCounter, 0, NULL, NULL )
		||	SQL_ERROR == SqlStmt_BindColumn( stmt, 3, SQLDT_INT, &finished, 0, NULL, NULL ) )
			SqlStmt_ShowDebug( stmt );

		while( SQL_SUCCESS == SqlStmt_NextRow( stmt ) ) {
			p->achieveData[achieveIndex].AchieveID = achieveID;
			p->achieveData[achieveIndex].count[achieveRequireIndex] = achieveCounter;
			p->achieveData[achieveIndex].update = false;
			if (finished > 0) {
				p->achieveData[ achieveIndex ].finished = true;
			}

		}
		strcat( t_msg, " achievements" );
	}
...
int delete_char_sql(int char_id)
{
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `char_id`='%d'", reg_db_char, char_id) )
		Sql_ShowDebug(sql_handle);
	...
	/* explored regions & dungeons */
	if( SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `char_region` WHERE `AccountID` = '%d'", scdata_db, account_id) )
		Sql_ShowDebug(sql_handle);
	if( SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `char_dungeon` WHERE `AccountID` = '%d'", scdata_db, account_id) )
		Sql_ShowDebug(sql_handle);
	
	/* achievements */
	if( SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `char_achievements` WHERE `CharID` = '%d'", scdata_db, char_id) )
		Sql_ShowDebug(sql_handle);
...
//					result = 2; // 2-gm level too low
...

		{
			...
			{// auth ok
				...
				WFIFOW(fd,0) = 0x2afd;
				WFIFOW(fd,2) = (uint16)(24 + sizeof(struct mmo_charstatus));
...
void sql_config_read(const char* cfgName)
{
	...
	while(fgets(line, sizeof(line), fp))
	{
		...
		else if(!strcmpi(w1,"reg_db_char"))
			safestrncpy(reg_db_char, w2, sizeof(reg_db_char));
		else if(!strcmpi(w1,"reg_db_login"))
			safestrncpy(reg_db_login, w2, sizeof(reg_db_login));
