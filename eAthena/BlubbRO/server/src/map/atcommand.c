...
char atcommand_symbol2 = '!'; // first char of the commands (2)
...
bool is_atcommand(const int fd, struct map_session_data* sd, const char* message, int type)
{
	...
	if( info == NULL || info->func == NULL || ( type && (((*atcmd_msg == atcommand_symbol || *atcmd_msg == atcommand_symbol2) && pc_isGM(sd) < info->level) || (*atcmd_msg == charcommand_symbol && pc_isGM(sd) < info->level2)) ) )
	{
	...
	//Log atcommands
	if( log_config.gm && info->level >= log_config.gm && (*atcmd_msg == atcommand_symbol || *atcmd_msg == atcommand_symbol2) )
...
int atcommand_config_read(const char* cfgName)
{
	...
	while( fgets(line, sizeof(line), fp) )
	{
		...
		if( strcmpi(w1, "command_symbol2") == 0 &&
			w2[0] > 31   && // control characters
			w2[0] != '/' && // symbol of standard ragnarok GM commands
			w2[0] != '%' && // symbol of party chat speaking
			w2[0] != '$' && // symbol of guild chat speaking
