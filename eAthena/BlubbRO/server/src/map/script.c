...
// [GodLesZ] custom commands :P


/// cap_value( <value>, <min>, <max> );
/// Caps the given value via C's cap_value
BUILDIN_FUNC(cap_value) {
	int value = script_getnum(st, 2);
	int min = script_getnum(st, 3);
	int max = script_getnum(st, 4);

	script_pushint(st, cap_value(value, min, max));
	return 0;
}

/// define( "<var_name>", <int value> );
/// Define's constants like in const.txt
BUILDIN_FUNC(define){
	const char *name;
	int value, n;
...
BUILDIN_FUNC( getmapname ) { 
	const char *name = script_getstr( st, 2 );
	int index = mapindex_name2id( name );
	if( index == -1 ) {
		script_pushstr( st, "-1" );
		return -1;
	}

	script_pushstr( st, map[ index ].name_display );
...
BUILDIN_FUNC(join_bg){
	...
	script_pushint(st, bg_team_join(bg_id, sd));
...
BUILDIN_FUNC(getip){
	...
	script_pushconststr(st, sd->ipstr);
 ...
 *
 **/
BUILDIN_FUNC( getachieveinarray ) { 
	int achieveID = script_getnum( st, 3 );
	int achieveType = script_getnum( st, 4 );
	struct AchievementObject *ach = AchieveDB( achieveType, achieveID );
	struct script_data* data;
	const char* name;
	char tmpVar[255];
	int32 id;
	...
	id = reference_getid( data );
	name = reference_getname( data );
	if( not_array_variable( *name ) || !is_string_variable( name ) ){
		st->state = END;
		return 1;
	}
	...
	sprintf( tmpVar, "%d", ach->ID );
	set_reg( st, sd, reference_uid( id, 0 ), name, ( void* )tmpVar, reference_getref( data ) );
	sprintf( tmpVar, "%d", ach->Type );
	set_reg( st, sd, reference_uid( id, 1 ), name, ( void* )tmpVar, reference_getref( data ) );
	set_reg( st, sd, reference_uid( id, 2 ), name, ( void* )ach->Name, reference_getref( data ) );
	set_reg( st, sd, reference_uid( id, 3 ), name, ( void* )ach->NameCutin, reference_getref( data ) );
	set_reg( st, sd, reference_uid( id, 4 ), name, ( void* )ach->Description, reference_getref( data ) );

	return 0;
}

BUILDIN_FUNC( getachieverequire ) { 
	int achieveID = script_getnum( st, 3 );
	int achieveType = script_getnum( st, 4 );
	struct AchievementObject *ach = AchieveDB( achieveType, achieveID );
	struct script_data* data;
	...
	/*
	if( ach->RequireObject.ID == 0 || ach->RequireObject.Count == 0 )
		return 0;

	for( i = 0; i < MAX_ACHIEVE_PARAMS; i++ ) {
		if( ach->RequireObject.Param[ i ] == 0 ) // end of Data
			break;
		
		set_reg( st, sd, reference_uid( id, j++ ), name, ( void* )ach->RequireObject.Param[ i ], reference_getref( data ) );
	}
	*/
	script_pushint( st, j );
	return 0;
...
BUILDIN_FUNC( getachievename ) { 
	int achieveID = script_getnum( st, 2 );
	int achieveType = script_getnum( st, 3 );
	struct AchievementObject *ach = AchieveDB( achieveType, achieveID );
	if( ach == NULL ) 
		script_pushstrcopy( st, "NULL" );
	else
		script_pushstrcopy( st, ach->Name );

	return 0;
}

BUILDIN_FUNC( getachievecutin ) { 
	int achieveID = script_getnum( st, 2 );
	int achieveType = script_getnum( st, 3 );
	struct AchievementObject *ach = AchieveDB( achieveType, achieveID );
	if( ach == NULL ) 
		...
		script_pushstrcopy( st, ach->NameCutin );

	return 0;
}

BUILDIN_FUNC( achieveadd ) { 
	int achieveType = script_getnum( st, 2 );
	int param1 = ( script_hasdata( st, 3 ) ? script_getnum( st, 3 ) : -66 );
	...
	if( param1 != -66 )
		achieve_CheckType( script_rid2sd( st ), achieveType, param1 );
	else
		achieve_CheckType( script_rid2sd( st ), achieveType );
	script_pushint( st, 0 );

	return 0;
}

BUILDIN_FUNC( achieveaddid ) { 
	struct AchievementObject *Achieve;
	int achieveID = script_getnum(st, 2);
	int achieveType = script_getnum(st, 2);
	if((Achieve = AchieveDB(achieveType, achieveID)) == NULL) {
		script_pushint( st, -1 );
		return 1;
	}

	achieve_GetReceive(script_rid2sd( st ), Achieve);
	script_pushint( st, 1 );
...
/// GetGStorageAccess( <guild_id>, <pos_index> );
BUILDIN_FUNC( getgstorageaccess ) {
	struct guild *g;
	int pos = script_getnum( st, 3 );
	if((g = guild_search( script_getnum( st, 2 ) )) == NULL){
		script_pushint( st, (int)GP_NONE );
		return 0;
	}

	script_pushint( st, (int)guild_GetStorageAccess( g, pos ) );
	return 0;
}

/// SetGStorageAccess( <guild_id>, <pos_index>, <access_level> );
BUILDIN_FUNC( setgstorageaccess ) {
	struct guild *g;
	if((g = guild_search( script_getnum( st, 2 ) )) == NULL)
		return 0;

	guild_SetStorageAccess( g, script_getnum( st, 3 ), (GuildPositionRank)script_getnum( st, 4 ) );
	return 0;
}

/// GetGuildPositionAccess( <guild_id>{,<pos_index>} );
BUILDIN_FUNC( getguildpositionaccess ){
	struct guild *g;
	int i, j = 0;
	...
	if (script_hasdata(st, 3)) {
		script_pushint( st, (int)guild_GetStorageAccess( g, script_getnum(st, 3) ) );
		return 0;
	}

	// request all
	for( i = 0; i < MAX_GUILDPOSITION; i++ ){
		mapreg_setregstr( add_str( "$@GuildPosName$" ) + ( j << 24 ), g->position[ i ].name );
		mapreg_setreg( add_str( "$@GuildPosAccess" ) + ( j << 24 ), (int)g->position[ i ].Access );

		j++;
	}

	mapreg_setreg( add_str( "$@GuildPosCount" ), j );
	return 0;
}

/// GetGuildPosition( {<char_id>} );
BUILDIN_FUNC( getguildposition ){
	struct map_session_data *sd = NULL;
	int pos = -1;

	if (script_hasdata(st, 2) && (sd = map_id2sd(script_getnum(st, 2))) == NULL) {
	...
	pos = guild_getposition(NULL, sd);
	script_pushint(st, pos);

	return 0;
}

/// GetStorage( {<char_id>{, <start_index>}} );
BUILDIN_FUNC( getstorage ){
	struct map_session_data *sd = NULL;
	int j = 0, i = 0, end, start = (script_hasdata(st, 3) ? script_getnum(st, 3) : 0);
	...
	if (start >= sd->status.storage.storage_amount) {
		script_pushint(st, 0);
		return 0;
	}
	
	end = start + 127;
	if (end >= sd->status.storage.storage_amount) {
		end = sd->status.storage.storage_amount;
	}

	for (i = start; i < end; j++, i++) {
		pc_setreg(sd, add_str("@StorageItemID") + (j << 24), sd->status.storage.items[i].nameid);
		pc_setreg(sd, add_str("@StorageItemAmount") + (j << 24), sd->status.storage.items[i].amount);
	}

	script_pushint(st, end - start);
...
BUILDIN_FUNC(setp){
	TBL_PC* sd = NULL;
	struct script_data* data;
	int num, result = 0, pid = 0;
	const char* name;
	char prefix;

	pid = script_getnum(st, 2);
	if ((sd = map_charid2sd(pid)) == NULL && (sd = map_id2sd(pid)) == NULL) {
		ShowError("script:setp: player with id %d not found\n", pid);
		script_reportdata(script_getdata(st, 2));
		st->state = END;
		return 1;
	}
	...
	data = script_getdata(st, 3);
	if (!data_isreference(data)){
		ShowError("script:setp: not a variable\n");
		script_reportdata(script_getdata(st, 3));
		st->state = END;
		return 1;
	}
	...
	if (!not_server_variable(prefix)){
		ShowError("script:setp: not a player variable\n");
		script_reportdata(script_getdata(st, 3));
		st->state = END;
		return 1;
	}
	...
	if (is_string_variable(name) == true){
		// string 
		const char* str = script_getstr(st, 4);
		if (prefix == '@') {
			result = pc_setregstr(sd, num, str);
		} else {
			result = pc_setglobalreg_str(sd, name, str);
		}
	} else {
		// integer 
		...
		// player constant?
		if (str_data[(num & 0x00ffffff)].type == C_PARAM){
			if ((result = pc_setparam(sd, str_data[(num & 0x00ffffff)].val, val)) == 0){
				ShowError("script:setp: failed to set param '%s' to %d.\n", name, val);
				script_reportsrc(st);
				st->state = END;
				return 1;
			}
			...
			if (prefix == '@') {
				result = pc_setreg(sd, num, val);
			} else {
				result = pc_setglobalreg(sd, name, val);
			}
		}
	}
...
BUILDIN_FUNC(getp){
	...
	pid = script_getnum(st, 2);
	if ((sd = map_charid2sd(pid)) == NULL && (sd = map_id2sd(pid)) == NULL) {
		ShowError("script:getp: player with id %d not found\n", pid);
		script_reportdata(script_getdata(st, 2));
		st->state = END;
		return 1;
	}
	...
	if (data_isreference(data) == false){
		ShowError("script:getp: not a variable\n");
		script_reportdata(script_getdata(st, 3));
		st->state = END;
		return 1;
	}
	...
	if (not_server_variable(prefix) == false){
		ShowError("script:getp: not a player variable\n");
		script_reportdata(script_getdata(st, 3));
		st->state = END;
		return 1;
	}
	...
	if (is_string_variable(name) == true){
		...
		char *str;
		if (prefix == '@'){
			str = pc_readregstr(sd, data->u.num);
		} else {
			str = pc_readglobalreg_str(sd, name);
		}
		...
		int val;
		if (reference_toconstant(data)){
			val = reference_getconstant(data);
		} else if (reference_toparam(data)){
			val = pc_readparam(sd, reference_getparamtype(data));
		} else if (prefix == '@') {
			val = pc_readreg(sd, data->u.num);
		} else {
			val = pc_readglobalreg(sd, name);
		}
...
BUILDIN_FUNC(exploredregion){
	...
	regID = map[m].flag.region;
	script_pushint(st, (regID < MAX_REGION ? sd->status.region[regID] == true : 0));

	return 0;
}

BUILDIN_FUNC(exploreddungeon){
	int m, dunID;
	const char *str = script_getstr(st, 2);
	TBL_PC *sd = script_rid2sd(st);
	if (sd == NULL) {
	...
	dunID = map[m].flag.dungeon;
	script_pushint(st, (dunID < MAX_DUNGEON ? sd->status.dungeon[dunID] == true : 0));
...
BUILDIN_FUNC(getdmglog){
	...
	for (i = 0; i < DAMAGELOG_SIZE; i++) {
		if (sd->dmglog[i].srcID == 0) {
			break;
		}
		pc_setreg(sd, reference_uid(add_str("@dmglog_id"), i), sd->dmglog[i].srcID);
		pc_setreg(sd, reference_uid(add_str("@dmglog_type"), i), sd->dmglog[i].srcType);
		pc_setreg(sd, reference_uid(add_str("@dmglog_dmg"), i), sd->dmglog[i].dmg);
	}
...
struct script_function buildin_func[] = {
	...
	BUILDIN_DEF(define, "si"),
	BUILDIN_DEF(substr, "sii"),
	BUILDIN_DEF(explode, "rss"),
	BUILDIN_DEF(join_bg, "i"),
	BUILDIN_DEF(getip, ""),
	BUILDIN_DEF(getachieveinarray, "rii"),
	BUILDIN_DEF(getachieverequire, "rii*"),
	BUILDIN_DEF(getachievename, "ii"),
	BUILDIN_DEF(getachievecutin, "ii"),
	BUILDIN_DEF(achieveadd, "i*"),
	BUILDIN_DEF(achieveaddid, "i"),
	BUILDIN_DEF(getgstorageaccess, "ii"),
	BUILDIN_DEF(setgstorageaccess, "iii"),
	BUILDIN_DEF(getguildpositionaccess, "i*"),
	BUILDIN_DEF(getguildposition, "*"),
	BUILDIN_DEF(getstorage, "*"),
	BUILDIN_DEF(setp,"iii"),
	BUILDIN_DEF(getp,"ii"),
	BUILDIN_DEF(specialeffectmvp,""),
	BUILDIN_DEF(exploredregion, "s"),
	BUILDIN_DEF(exploreddungeon, "s"),
	BUILDIN_DEF(getnpcinfo, "i?"),
	BUILDIN_DEF(getdmglog, ""),
