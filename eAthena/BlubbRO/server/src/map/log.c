...
// min / max preis checking Helper
// `index`, `item_id`, `refine`, `card0`, `card1`, `card2`, `card3`, `min`, `max`, `v_min`, `v_max`
int check_price( struct item *item, int preis, char *row_min, char *row_max ) {
	int item_id = item->nameid;
	if( Sql_Query( mmysql_handle, "SELECT `index`, `item_id`, `refine`, `card0`, `card1`, `card2`, `card3`, `%s`, `%s` FROM `item_price_db` WHERE ( `item_id` = '%d' AND `refine` = '%d' AND `card0` = '%d' AND `card1` = '%d' AND `card2` = '%d' AND `card3` = '%d' ) LIMIT 0,1", row_min, row_max, item->nameid, item->refine, item->card[ 0 ], item->card[ 1 ], item->card[ 2 ], item->card[ 3 ] ) != SQL_SUCCESS ) {
		...
		if( Sql_NextRow( mmysql_handle ) == SQL_SUCCESS ){
			tmp = ( struct item* )aCalloc( 1, sizeof( struct item ) );
			Sql_GetData( mmysql_handle, 0, &data, NULL ); index = atoi( data );
			Sql_GetData( mmysql_handle, 1, &data, NULL ); tmp->nameid = atoi( data );
			Sql_GetData( mmysql_handle, 2, &data, NULL ); tmp->refine = atoi( data );
			Sql_GetData( mmysql_handle, 3, &data, NULL ); tmp->card[ 0 ] = atoi( data );
			Sql_GetData( mmysql_handle, 4, &data, NULL ); tmp->card[ 1 ] = atoi( data );
			Sql_GetData( mmysql_handle, 5, &data, NULL ); tmp->card[ 2 ] = atoi( data );
			Sql_GetData( mmysql_handle, 6, &data, NULL ); tmp->card[ 3 ] = atoi( data );
			Sql_GetData( mmysql_handle, 7, &data, NULL ); min = atoi( data );
			Sql_GetData( mmysql_handle, 8, &data, NULL ); max = atoi( data );
		}

		if( tmp == NULL ) { // tmp == NULL, heisst kein draw aus der table, => adden
			if( SQL_ERROR == Sql_Query( mmysql_handle, "INSERT DELAYED INTO `item_price_db` ( `item_id`, `refine`, `card0`, `card1`, `card2`, `card3`, `%s`, `%s` ) VALUES ( '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d' )", row_min, row_max, item->nameid, item->refine, item->card[ 0 ], item->card[ 1 ], item->card[ 2 ], item->card[ 3 ], preis, preis ) )
				Sql_ShowDebug( mmysql_handle );
		} else {
			if( preis < min ) {
				if( SQL_ERROR == Sql_Query( mmysql_handle, "UPDATE `item_price_db` SET `%s` = '%d' WHERE `index` = '%d'", row_min, preis, index ) )
					Sql_ShowDebug( mmysql_handle );
			} 
			if( preis > max ) {
				if( SQL_ERROR == Sql_Query( mmysql_handle, "UPDATE `item_price_db` SET `%s` = '%d' WHERE `index` = '%d'", row_max, preis, index ) )
					Sql_ShowDebug( mmysql_handle );
			}
		}
...
// für CP -> Live Shop anzeige

// flag 0 => add to Shop
// flag 1 => clear Log
int log_shop( struct map_session_data *sd, int flag ) {
	int i, idx = -1;
	char esc_name[ NAME_LENGTH * 2 + 1 ], esc_shopname[ sizeof( sd->message ) * 2 + 1 ];
	nullpo_retr( 0, sd );

	Sql_EscapeString( mmysql_handle, esc_name, sd->status.name );
	Sql_EscapeString( mmysql_handle, esc_shopname, sd->message );


	time( &curtime );
	strftime( timestring, 254, "%d.%m. %H:%M:%S", localtime( &curtime ) );

	if( flag ) {
		if( Sql_Query( mmysql_handle, "DELETE FROM `shop_log` WHERE `char_id` = '%d'", sd->status.char_id ) != SQL_SUCCESS )
			Sql_ShowDebug( mmysql_handle );
		if( Sql_Query( mmysql_handle, "DELETE FROM `shop_log_item` WHERE `char_id` = '%d'", sd->status.char_id ) != SQL_SUCCESS )
			Sql_ShowDebug( mmysql_handle );

		return 1;
	}

	// `vender_id`, `char_id`, `char_name`, `shop_name`, `map`, `x`, `y`, `date`
	if( Sql_Query( mmysql_handle, 
		"INSERT INTO `shop_log` ( "
		"`char_id`, `char_name`, `shop_name`, `map`, `x`, `y`, `date` ) VALUES ( "
		"'%d', '%s', '%s', '%s', '%d', '%d', '%s' )",
		sd->status.char_id, esc_name, esc_shopname, mapindex_id2name( sd->mapindex ), sd->bl.x, sd->bl.y, timestring ) != SQL_SUCCESS
	 )
		Sql_ShowDebug( mmysql_handle );

	for( i = 0; sd->vending[ i ].amount; i++ ) {
		// `char_id`, `item_id`, `count`, `preis`, `refine`, `card0`, `card1`, `card2`, `card3`
		idx = sd->vending[ i ].index;
		if( Sql_Query( mmysql_handle, 
			"INSERT INTO `shop_log_item` ( "
			"`char_id`, `item_id`, `count`, `preis`, `refine`, `card0`, `card1`, `card2`, `card3` ) VALUES ( "
			"'%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d' )",
			sd->status.char_id, sd->status.cart[ idx ].nameid, sd->vending[ i ].amount, sd->vending[ i ].value, 
			sd->status.cart[ idx ].refine,
			sd->status.cart[ idx ].card[ 0 ],sd->status.cart[ idx ].card[ 1 ],sd->status.cart[ idx ].card[ 2 ],sd->status.cart[ idx ].card[ 3 ] ) != SQL_SUCCESS
		 )
			Sql_ShowDebug( mmysql_handle );

		// update min/max Price [not sold]
		check_price( &sd->status.cart[ idx ], sd->vending[ i ].value, "min", "max" );
	}
...
int log_dungeonexplore( struct map_session_data *sd, struct dungeon *d ){
	FILE* logfp;
	if( !log_config.enable_logs )
		return 0;
	...
	if( ( logfp = fopen( "log/dungeon_explore.log", "a+" ) ) == NULL )
		return 0;
	time( &curtime );
	strftime( timestring, 254, "%d.%m.%Y %H:%M:%S", localtime( &curtime ) );
	fprintf( logfp, "%s - %s[%d:%d] [%d/%d]\t%s (%d Maps)\n", timestring, sd->status.name, sd->status.account_id, sd->status.char_id, sd->status.base_level, sd->status.job_level, d->name, d->map_count );
	fclose( logfp );

	return 1;
}

int log_regionexplore( struct map_session_data *sd, struct region *r ){
	FILE* logfp;
	if( !log_config.enable_logs )
		return 0;
	...
	if( ( logfp = fopen( "log/region_explore.log", "a+" ) ) == NULL )
		return 0;
	time( &curtime );
	strftime( timestring, 254, "%d.%m.%Y %H:%M:%S", localtime( &curtime ) );
	fprintf( logfp, "%s - %s[%d:%d] [%d/%d]\t%s (%d Maps)\n", timestring, sd->status.name, sd->status.account_id, sd->status.char_id, sd->status.base_level, sd->status.job_level, r->name, r->map_count );
	fclose( logfp );
