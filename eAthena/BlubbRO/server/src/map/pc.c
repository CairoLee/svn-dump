...
bool pc_authok(struct map_session_data *sd, int login_id2, time_t expiration_time, int gmlevel, struct mmo_charstatus *st)
{
	...
	sprintf(sd->ipstr, "%d.%d.%d.%d", CONVIP(ip));
...
void pc_log_damage(struct map_session_data *sd, struct block_list *src, int damage) {
	...
	{ // Log damage...
		...
		for (i = 0; i < DAMAGELOG_SIZE; i++) {
			if (sd->dmglog[i].srcID == char_id && sd->dmglog[i].srcType == srcType) {
				sd->dmglog[i].dmg += damage;
				i = DAMAGELOG_SIZE; // break it
				break;
			}
		...
		if (i < DAMAGELOG_SIZE) {
			sd->dmglog[i].srcID  = char_id;
			sd->dmglog[i].srcType= srcType;
			sd->dmglog[i].dmg = damage;
		}
	}
	return;
...
/**********************************
 * neue Region entdeckt
 **********************************/
int pc_explore_region( struct map_session_data *sd, int index ) {
	char output[256];
	unsigned int exp;
	unsigned int jexp;
	struct region *r = region_db( index );
	int m = map_mapname2mapid( region_data[ index ]->map_name );

	if( sd->status.region[index] == true || r == NULL )
		return -1;

	if( m != sd->bl.m ) { // Char muss die Hauptstadt betreten?
		sprintf( output, "[ Unknown region entered ]" ); // Message f¸r unbekannte Region
		clif_broadcast2( &sd->bl, output, strlen( output ) + 1, strtol("0x00CCFF", NULL, 0), 0x190, 12, 0, 0, SELF);
		return -1;
	}
	
	sprintf( output, "[ New region discovered: '%s' ]", r->name );
	sd->status.region[index] = true;
	clif_broadcast2( &sd->bl, output, strlen( output ) + 1, strtol("0xFE7A7A", NULL, 0), 0x190, 12, 0, 0, SELF);
	
	exp =  pc_nextbaseexp( sd );
	jexp = pc_nextjobexp( sd );
	exp  = (unsigned int)(((float)exp / 100.) * r->map_count);
	jexp = (unsigned int)(((float)jexp / 100.) * r->map_count);
	if( sd->gmlevel ){
		clif_displaymessagev( sd->fd, "Explore Debug: nextBase[%d] MapCount[%d], Result [%d]\n", pc_nextbaseexp( sd ), r->map_count, exp );
		clif_displaymessagev( sd->fd, "Explore Debug: nextJob[%d] MapCount[%d], Result [%d]\n", pc_nextjobexp( sd ), r->map_count, jexp );
	}
	if( exp || jexp )
		pc_gainexp( sd, NULL, exp, jexp, 0 );
	
	// Achievement - explore Region
	achieve_CheckType( sd, TACHIEVE_EXPLORE_REGION, index );

	log_regionexplore( sd, r );

	pc_setregstr( sd, add_str( "@region_name$" ), r->name );
	npc_script_event( sd, NPCE_EXPLOREREGION );
 ...
 * Region betreten
 **********************************/
int pc_join_region( struct map_session_data *sd, int index ) {
	struct guild *g;
	struct region *r = region_db( index );
	char output[ 256 ];
	...
	g = guild_search( r->guild_id );
	if( g == NULL ) // Income Message
		sprintf( output, "[ Region entered: %s ]", r->name );
	else
		sprintf( output, "[ Region entered: %s. Controlling Guild: %s]", r->name, g->name );
	clif_broadcast2( &sd->bl, output, strlen( output ) + 1, strtol("0x00CCFF", NULL, 0), 0x190, 12, 0, 0, SELF);
...
/**********************************
 * neues Dungeon entdeckt
 **********************************/
int pc_explore_dungeon( struct map_session_data *sd, int index ) {
	char output[256];
	struct dungeon *d = dungeon_db( index );
	unsigned int exp;
	unsigned int jexp;

	if( sd->status.dungeon[index] == true || d == NULL )
		return -1;
	
	sd->status.dungeon[index] = true;

	sprintf( output, "[ New Dungeon discovered: %s ]", d->name );
	clif_broadcast2(&sd->bl, output, strlen(output) + 1, strtol("0xFE7A7A", NULL, 0), 0x190, 12, 0, 0, SELF);	

	exp  = pc_nextbaseexp( sd );
	jexp = pc_nextjobexp( sd );
	exp  = (unsigned int)(((float)exp / 100.) * d->map_count);
	jexp = (unsigned int)(((float)jexp / 100.) * d->map_count);
	if( sd->gmlevel ) {
		clif_displaymessagev( sd->fd, "Explore Debug: nextBase[%d] MapCount[%d], Result [%d]\n", pc_nextbaseexp( sd ), d->map_count, exp );
		clif_displaymessagev( sd->fd, "Explore Debug: nextJob[%d] MapCount[%d], Result [%d]\n", pc_nextjobexp( sd ), d->map_count, jexp );
	}
	if( exp || jexp )
		pc_gainexp( sd, NULL, exp, jexp, 0 );

	// Achievement - explore Dungeon
	achieve_CheckType( sd, TACHIEVE_EXPLORE_DUNGEON, index );
	
	log_dungeonexplore( sd, d );

	pc_setregstr( sd, add_str( "@dungeon_name$" ), d->name );
	npc_script_event( sd, NPCE_EXPLOREDUNGEON );
	return 0;
}
 ...
 * Dungeon betreten
 **********************************/
int pc_join_dungeon( struct map_session_data *sd, int index ) {
	struct dungeon *d = dungeon_db( index );
	char output[ 256 ];
	...
	sprintf( output, "[ Dungeon entered: %s ]", d->name );
	clif_broadcast2( &sd->bl, output, strlen( output ) + 1, strtol("0x00CCFF", NULL, 0), 0x190, 12, 0, 0, SELF);
...
/// Checks if the Player may warp to the Region or Dungeon
/// Now, returns 0 if Region/Dungeon Data failed or Player knows the Place
/// Returns 1 if WarpCheck::OnWarp NPC got called
int pc_check_warp( struct map_session_data *sd, int m, int x, int y ) {
	struct dungeon *d = dungeon_db( map[ m ].flag.dungeon );
	struct region *r = region_db( map[ m ].flag.region );

	if( d == NULL && r == NULL )
		return 0;

	if( r && ( r->state != true || sd->status.region[ r->index ] == true ) ) 
		return 0;
	if( d && ( d->state != true || sd->status.dungeon[ d->index ] == true ) ) 
		return 0;

	// call NPC
	pc_setregstr( sd, add_str( "@WarpCheckMap$" ), map[ m ].name );
	pc_setreg( sd, add_str( "@WarpCheckMapX" ), x );
	pc_setreg( sd, add_str( "@WarpCheckMapY" ), y );
	npc_event( sd, "WarpCheck::OnWarp", 0 );
