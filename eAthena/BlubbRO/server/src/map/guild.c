...
GuildPositionRank guild_GetStorageAccess( struct guild *g, int pos ) {
	nullpo_retr( GP_NONE, g );
	if( pos < 0 || pos >= MAX_GUILDPOSITION )
		return GP_NONE;
	return g->position[ pos ].Access;
}

void guild_SetStorageAccess( struct guild *g, int pos, GuildPositionRank NewLevel ) {
	nullpo_retv( g );
	if( pos < 0 || pos >= MAX_GUILDPOSITION )
		return;
	g->position[ pos ].Access = NewLevel;
}
