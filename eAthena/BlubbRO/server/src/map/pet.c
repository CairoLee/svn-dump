...
static int pet_hungry(int tid, unsigned int tick, int id, intptr data)
{
	...
	if (pd->pet.hungry >= 0 && pd->pet.hungry <= 1) {
		clif_pet_emotion(sd->pd, 7); // Smoke cloud (angry) /ag
		snprintf(temp, sizeof temp, "*~ %s, Goes To Order Pizza ~*", pd->pet.name);
		clif_displaymessage(sd->fd, temp);
	} else if (pd->pet.hungry <= 5) {
		clif_pet_emotion(sd->pd, 16); // Wah (crying) /wah
		if (pd->pet.hungry == 5) {
			snprintf(temp, sizeof temp, "*~ %s, Begins to Starve ~*", pd->pet.name);
			clif_displaymessage(sd->fd, temp);
		}
	} else if (pd->pet.hungry <= 15) {
		clif_pet_emotion(sd->pd, 43); // Obsessed 2 (eyes popping) /e8 or /slur
		if (pd->pet.hungry == 15) {
			snprintf(temp, sizeof temp, "*~ %s, Gets Hungrier ~*", pd->pet.name);
			clif_displaymessage(sd->fd, temp);
		}
	} else if (pd->pet.hungry <= 25) {
		clif_pet_emotion(sd->pd, 37); // Obsessed 1 (drooling) /e2 or /rice
		if (pd->pet.hungry == 25) {
			snprintf(temp, sizeof temp, "*~ %s, Is Now Hungry ~*", pd->pet.name);
			clif_displaymessage(sd->fd, temp);
		}
	} else if (pd->pet.hungry <= 30) {
		if (pd->pet.hungry%2 == 0){
			clif_pet_emotion(sd->pd, 20); // Hmm /hmm
		}
		if (pd->pet.hungry == 30) {
			snprintf(temp, sizeof temp, "*~ %s, Looks At You ~*", pd->pet.name);
			clif_displaymessage(sd->fd, temp);
		}
	}
