...
int map_readallmaps (void)
{
	...
	for(i = 0; i < map_num; i++)
	{
		...
		if( Sql_Query( mmysql_handle, "SELECT `NameDisplay` FROM `map_db` WHERE `Name` = '%s'", map[ i ].name ) != SQL_SUCCESS ){
			Sql_ShowDebug( mmysql_handle );
			strncpy(map[ i ].name_display, map[ i ].name, strlen(map[ i ].name));
		} else {
			if( Sql_NextRow( mmysql_handle ) == SQL_SUCCESS ) {
				char *data;
				Sql_GetData( mmysql_handle, 0, &data, NULL );

				strncpy(map[ i ].name_display, data, strlen(data));
			}
		}
	}
