...
{	//Holds pointers to the commonly executed scripts for speedup. [Skotlex]
	struct event_data *event[SHRT_MAX];
	const char *event_name[SHRT_MAX];
	short event_count;
...
static const char* npc_parse_mapflag(char* w1, char* w2, char* w3, char* w4, const char* start, const char* buffer, const char* filepath)
{
	...
	else if( !strcmpi( w3,"region" ) ) {
		state = ( w4 ? atoi( w4 ) : -1 );
		if( region_db( state ) == NULL ) {
			ShowError( "npc_parse_mapflag: region: %d hat keine region_data\n", state );
			return strchr( start,'\n' );
		} 
		if( state ) 
			region_data[ state ]->map_count++;
		map[ m ].flag.region = ( state ? state : 0 );
	} else if( !strcmpi( w3,"dungeon" ) ) {
		state = ( w4 ? atoi( w4 ) : -1 );
		if( dungeon_db( state ) == NULL ) {
			ShowError( "npc_parse_mapflag: dungeon: %d hat keine dungeon_data\n", state );
			return strchr( start,'\n' );
		}
		if( state ) 
			dungeon_data[ state ]->map_count++;
		map[ m ].flag.dungeon = ( state ? state : 0 );
	} 
	else
...
void npc_read_event_script(void)
{
	...
	struct {
		...
		{"Change Region Event",script_config.changeregion_event_name},
		{"Change Dungeon Event",script_config.changedungeon_event_name},
		{"Change Map Event",script_config.changemap_event_name},
		{"Explore Region Event",script_config.exploreregion_event_name},
		{"Explore Dungeon Event",script_config.exploredungeon_event_name},
		{"Kill Mob Party Event",script_config.kill_mob_party_event_name}
	};
	...
	for (i = 0; i < NPCE_MAX; i++)
	{
		...
		for( data = iter->first(iter,&key); iter->exists(iter); data = iter->next(iter,&key) )
		{
			...
			short count = script_event[i].event_count;
