...
static bool account_db_sql_check_ip(AccountDB* self, struct mmo_account* acc, char* ip);
...
AccountDB* account_db_sql(void)
{
	...
	db->vtable.check_ip     = &account_db_sql_check_ip;
	...
	safestrncpy(db->accreg_db, "login_reg_global", sizeof(db->accreg_db));
...
static bool account_db_sql_check_ip(AccountDB* self, struct mmo_account* acc, char* ip)
{
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT l.`account_id` FROM `login` AS l, `char` AS c WHERE c.`account_id` = l.`account_id` AND c.`online` = 1 AND l.`double_login_allowed` = 0 AND l.`account_id` != '%d' AND l.`last_ip` = '%s'", acc->account_id, ip) )
	{
		Sql_ShowDebug(sql_handle);
		return false;
	}
...
static bool mmo_auth_fromsql(AccountDB_SQL* db, struct mmo_account* acc, int account_id)
{
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `str`,`value` FROM `%s` WHERE `account_id`='%d'", db->accreg_db, acc->account_id) )
	{
		Sql_ShowDebug(sql_handle);
		return false;
	}
...
static bool mmo_auth_tosql(AccountDB_SQL* db, const struct mmo_account* acc, bool is_new)
{
	...
	if( SQL_SUCCESS != Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `account_id`='%d'", db->accreg_db, acc->account_id) )
	{
		Sql_ShowDebug(sql_handle);
		break;
	}
	// insert new account regs
	if( SQL_SUCCESS != SqlStmt_Prepare(stmt, "INSERT INTO `%s` (`account_id`, `str`, `value`) VALUES ( '%d' , ? , ? );",  db->accreg_db, acc->account_id) )
	{
		SqlStmt_ShowDebug(stmt);
		break;
	}
